...
// -------------------------------------------------------------- 用户偏好契约
//
// `/prefs` 的 wire 形状（2026-10 起的新契约，一次到位）：
//
//   { "pinned_app_ids": ["id1", "id2"], "last_place_by_app": { "id1": "drawer" } }
//
//   * `pinned_app_ids` —— 固定到会话标题栏的小程序（**可以多个**，按用户固定的先后排序）。
//   * `last_place_by_app` —— 每个小程序上一次被用户摆进的**呈现面**（panel / drawer /
//     session / corner 四键之一；browser 不是"呈现面"，它开的是 DSH 之外的新页签）。
//
// 为什么形状归一放在 `lib/index.js` 而不是 `lib/store.js`：store 是纯存储层（读原始
// JSON、写原始 JSON），而迁移与校验是**契约**的事 —— 契约变了只改这一处，store 的
// 读写原语一行不动。

/**
 * 标题栏可固定的小程序数量上限。
 *
 * 为什么是 8：标题栏是整条会话头部里最挤的一行（会话标题 + 日志按钮都在抢宽度），
 * 胶囊主段里 8 枚 20px 图标格（160px）已经是"不挤掉会话标题"的边界；再多就该靠
 * 「全部小程序」列表而不是把标题栏铺满。客户端那一半也有一份同值常量
 * （`PINNED_MAX`，见 lib/client.js），两边由测试钉住相等 —— wire 契约两边各写一份
 * 是既成格局（如 IFRAME_SANDBOX），对齐靠测试而不是靠人记。
 */
export const PREFS_PINNED_MAX = 8

/**
 * `last_place_by_app` 的值白名单：`LAYOUT_PLACES`（客户端那张表）里除 `browser` 外的四个键。
 *
 * browser 刻意不在名单里：那五个键回答"同一个小程序能跑到哪里去"，而这一个字段记录
 * "用户把它摆进了哪个**呈现面**" —— 浏览器新页签开的是 DSH 之外的一个副本，切走之后
 * 原来的面一个都没动，它不是"形态"，记下来只会让下次打开莫名其妙地去开新页签。
 */
export const LAST_PLACE_KEYS = Object.freeze(['panel', 'drawer', 'session', 'corner'])

/** 一份空的偏好（新装机 / 盘上读不到任何可用东西时的形状）。每次调用给新对象，调用方可放心改。 */
export function emptyPrefs() {
	return { pinned_app_ids: [], last_place_by_app: {} }
}

/**
 * 把任意一份"盘上或请求体里的东西"归一成新形状。
 *
 * 读路径与写路径**共用这一个函数**（而不是各写一份），理由是它们要认的其实是同一份
 * 契约：旧客户端 POST `{ pinned_app_id: "x" }` 与旧盘上的同一形状必须走同一条迁移，
 * 两边各写一份就会在某天只改了一边。
 *
 * 认三种历史状态：
 *  1. 旧形状 `{ pinned_app_id: "x" }` → 升级成 `{ pinned_app_ids: ["x"], last_place_by_app: {} }`；
 *  2. 新形状照读（垃圾项被过滤）；
 *  3. 坏 JSON / 形状不对 → 空（由调用方在 JSON.parse 那一层兜住，这里只兜"是对象但字段烂"）。
 *
 * 新旧键**同时**存在时新键优先（`pinned_app_ids` 是数组就按它读）：迁移是一次性的事，
 * 混合状态只会在"写入被旧版本回滚"时出现，那时以更完整的那个字段为准。
 *
 * 绝不抛错：prefs 只影响标题栏与"下次打开去哪儿"，任何烂输入都归成可用形状，
 * 而不是让端点 500（这是 `readPrefs` 一贯的策略：坏了不能拖垮插件）。
 */
export function normalisePrefsShape(raw) {
	const next = emptyPrefs()
	if (raw === null || typeof raw !== 'object' || Array.isArray(raw)) return next
	// 固定列表：只收合法 id，去重（保留第一次出现的顺序），截到上限。
	if (Array.isArray(raw.pinned_app_ids)) {
		for (const id of raw.pinned_app_ids) {
			if (typeof id !== 'string' || !isMiniAppId(id)) continue
			if (next.pinned_app_ids.includes(id)) continue
			if (next.pinned_app_ids.length >= PREFS_PINNED_MAX) break
			next.pinned_app_ids.push(id)
		}
	} else if (typeof raw.pinned_app_id === 'string' && isMiniAppId(raw.pinned_app_id)) {
		// 旧形状（单数）：读时升级成数组，写时只落新形状 —— 盘上那份旧文件在第一次
		// 写入之前一个字节都不动（见 /prefs 路由的说明）。
		next.pinned_app_ids.push(raw.pinned_app_id)
	}
	// 上次呈现面：key 必须是合法 id，value 只认四个呈现键；其余条目整个丢弃。
	const places = raw.last_place_by_app
	if (places !== null && typeof places === 'object' && !Array.isArray(places)) {
		for (const [key, value] of Object.entries(places)) {
			if (!isMiniAppId(key)) continue
			if (typeof value !== 'string' || !LAST_PLACE_KEYS.includes(value)) continue
			next.last_place_by_app[key] = value
		}
	}
	return next
}
...
/**
 * 读盘上的 prefs 并归一成新形状；读不到（没写过）/ 坏 JSON / 形状不对 → 一份空偏好。
 *
 * **绝不抛错、绝不 500**：prefs 只影响标题栏上那排图标与"下次打开去哪儿"，坏了就
 * 当"没有偏好"（这是与 index.json 刻意不同的策略 —— 那边是库的唯一权威，必须 fail
 * loud；这边拿大炮打蚊子）。`readJsonBody` 那一层已经保证 POST 的 body 是合法 JSON
 * 对象，这里再兜住"是 JSON 但字段烂"的那一半。
 */
async function readPrefsFile(store) {
	const { readFile } = await import('node:fs/promises')
	let text
	try {
		text = await readFile(store.prefsPath, 'utf8')
	} catch {
		return emptyPrefs()
	}
	try {
		return normalisePrefsShape(JSON.parse(text))
	} catch {
		return emptyPrefs()
	}
}
...
/**
 * 把归一后的新形状原子地写进 `store.prefsPath`，并把它原样交回去。
 *
 * 落盘的**永远**是归一结果：垃圾 id、超上限的尾巴、白名单之外的呈现键都在
 * `normalisePrefsShape` 里被滤掉 —— 端点不接受"把垃圾写进盘"这种结果，哪怕请求体
 * 本身是合法 JSON。`atomicWrite`（store.js 的原语）保证写到一半被杀时盘上是完整的
 * 旧文件或完整的新文件，不会是半个。
 */
async function writePrefsFile(store, body) {
	const next = normalisePrefsShape(body)
	await atomicWrite(store.prefsPath, JSON.stringify(next, null, '\t'))
	return next
}
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: viaPrefix(API_PREFIX, async (req, res, path, method) => {
			try {
				...
				// 用户偏好（标题栏固定哪几个小程序 + 每个小程序上次的呈现面）。
				//
				// 为什么不放客户端 localStorage：那不是 DSH 的插件契约，而"固定"是用户
				// 明确的意图、重载页面不该丢。宿主的 HTTP 通道 + dataDir 是现成的去处，
				// 也没有引入任何新的耦合。
				//
				// **读时迁移、写时落新形状**：GET 只把盘上任意历史状态归一成新形状交出去
				// （旧 `{pinned_app_id:"x"}` → `pinned_app_ids:["x"]`），**不回写** —— 盘上
				// 那份旧文件在第一次 POST 之前一个字节都不动；POST 则一律把归一结果写成
				// 新形状，从此盘上只有新形状。
				//
				// 不走 `store.readPrefs()/writePrefs()`：那两个是**旧契约**的存储原语
				// （只认单数 `pinned_app_id`，写时会丢掉别的字段），这里绕开它们直接读写
				// `store.prefsPath`，归一逻辑全在 `normalisePrefsShape` 一处。
				if (path === '/prefs') {
					if (method === 'GET') return json(res, 200, { ok: true, data: await readPrefsFile(store) })
					if (method === 'POST' || method === 'PATCH') {
						if (!isTrustedMutation(req)) {
							return json(res, 403, { ok: false, error: '请求来源不受信任' })
						}
						const body = await readJsonBody(req)
						return json(res, 200, { ok: true, data: await writePrefsFile(store, body) })
					}
