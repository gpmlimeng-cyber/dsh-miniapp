...
/**
 * 有界地取 `<title>` 的正文；取不到返回 undefined。
 *
 * **不要写回 `/<title[^>]*>([\s\S]*?)<\/title>/i`。** 那个正则在"有 `<title` 但没有
 * 闭合"的输入上是**二次**的：`[^>]*` 会从每一个位置一路扫到串尾。实测
 * `'<title'.repeat(20000)`（120 KB）就要 **3.5 秒**，4 MiB 是小时级 —— 而这是一条
 * **同步**路径（导入工具与 `POST /import` 都走它），期间整个 host 的事件循环、所有会话的
 * 流式输出会被一起钉死。用户点一下导入对话框就能走到（客户端只发 `{html, file_name}`）。
 *
 * 改成三次 `indexOf` 的有界扫描：线性、且只扫开头这一段。`<title>` 按规范在 `<head>` 里，
 * 正常文档里它离开头很近；超限就退回用文件名推断 —— 代价只是名字不够漂亮。
 */
const TITLE_SCAN_LIMIT = 256 * 1024

export function readDocumentTitle(html) {
	if (typeof html !== 'string' || html.length === 0) return undefined
	const window = html.length > TITLE_SCAN_LIMIT ? html.slice(0, TITLE_SCAN_LIMIT) : html
	// `<title>` 与 `<title lang="…">` 都要认，所以要求 `<title` 后面跟着空白或 `>`。
	// 这个模式没有嵌套量词，是线性的。
	const open = /<title[\s>]/i.exec(window)
	if (open === null) return undefined
	const gt = window.indexOf('>', open.index)
	if (gt < 0) return undefined
	const close = window.toLowerCase().indexOf('</title', gt + 1)
	if (close < 0) return undefined
	const title = window.slice(gt + 1, close).trim()
...
export function suggestName(html, fileName) {
	...
	if (title !== undefined) return clampName(title)
