...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var COPY = {
						zh: {
							...
							bar: {
								...
								// 左边那颗：有固定时是「打开谁」，没有固定时是「还没有固定」。
								openPinned: "打开「{name}」",
								noPinned: "还没有固定的小程序 —— 点右边那颗打开小程序栏",
								// 右边那颗 ▾，以及每一行末尾的 ⋮。
								more: "小程序栏",
								menu: "更多操作",
								pin: "固定到标题栏",
						...
						en: {
							...
							bar: {
								...
								noPinned: "Nothing pinned yet — use the button on the right to open the mini-app bar",
								more: "MiniApps bar",
								menu: "More actions",
		...
		var ICON_PATHS = {
			...
			/**
			 * 标题栏小程序栏上的五个图案。前两个是**同一个几何的两态**：📌 固定 / 未固定。
			 *
		 ...
		 *
		 * `stroke: true` 换成描边画法 —— 只有那三个"本身就该是一根线条"的图案用它
		 ...
		 * 读一次。
		 *
		 * 失败**不弹错**，只是当成"没有固定"：它影响的只有标题栏上的一颗图标，
		 * 没有任何理由为它弹一条错误、更没有理由挡住会话头部。`loaded` 照样置真 ——
		...
		// **order 升序即从左到右**。
		//
		// 参考 Chrome 的「扩展程序」那一栏：标题栏上两颗按钮（固定住的那一个 + ▾），
		// ▾ 下面挂一个下拉面板，面板自上而下是"固定的 / 全部 / 管理小程序"。
		//
		// 为什么是两颗而不是一颗：它们回答的是两个不同的问题 —— 左边那颗是"打开我钉住的那一个"，
		// 右边那颗是"看看还有哪些"。Chrome 也是这么分的（图标点开自己、箭头点开列表）。
		//
		...
		/** 面板与 ▾ 之间的缝、面板与视口边缘的最小间距。 */
		var BAR_PANEL_GAP = 6;
		var BAR_PANEL_MARGIN = 8;
		/** 量不到 ▾（ref 还没挂上、更老的 DSH）时的兜底坐标：标题栏下面一点、靠右一点。 */
		...
		/** 标题栏那颗按钮里的图案：固定的小程序画**它自己的 emoji**，没固定就画通用图标。 */
		function barPinnedIcon(app) {
			var icon = app === null || app === undefined ? "" : String(app.icon === null || app.icon === undefined ? "" : app.icon).trim();
			if (icon === "") return React.createElement(Icon, { name: "app", size: 16 });
		...
		/**
		 * 标题栏上的小程序栏：两颗按钮 + 它们那个下拉面板。
		 *
		 * 左边那颗 = **打开固定住的那一个**（用 `switchLayout("panel", …)`，也就是"打开使用"）；
		 * 一个都没固定时它显示通用图标、名字说明"还没有固定"，点它就打开面板
		 * （那一刻用户要的显然是"看看有哪些"）。右边那颗 = 开合面板。
		 *
		 * **固定的那一个已经不在目录里**（被删了）：界面上当作"没有固定"
		 * —— 画一颗打不开的图标比空着更糟。但**绝不去改 prefs**：
		 * 用户可能只是暂时删了它，下次再建一个同 id 的（或者他改主意了）就该原样回来。
		 */
		function MiniAppBar(props) {
			...
			var pinned = findCatalogApp(catalog, prefsState.pinnedAppId);
			...
			var onPinnedButton = function () {
				// 还没有固定：这颗按钮就是"看看有哪些"。
				if (pinned === null) { setOpen(!open); return; }
				setOpen(false);
				switchLayout("panel", pinned.miniapp_id, { t: t, ctx: ctx });
			};
			...
			return React.createElement("div", {
				...
				ref: barRef,
				// `flex: "0 0 auto"`：头部挤的时候该被压缩的是别的东西，不是这两颗按钮。
				style: { display: "flex", alignItems: "center", gap: 2, flex: "0 0 auto" }
			},
				React.createElement(ToolbarAction, {
					label: pinned === null ? t("bar.noPinned") : t("bar.openPinned", { name: pinned.name }),
					content: barPinnedIcon(pinned),
					attrs: { "data-dsh-miniapp-bar-part": "pinned" },
					onRun: onPinnedButton
				}),
				React.createElement(ToolbarAction, {
					label: t("bar.more"),
					content: React.createElement(Icon, { name: "chevron", size: 16, stroke: true }),
					attrs: {
						"data-dsh-miniapp-bar-part": "more",
