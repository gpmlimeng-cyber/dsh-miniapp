...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * 打开右栏 —— **两条通道都认**，返回我们实际用上的那一条。
		 *
		 * `sidebarRight`（新 DSH：0.1.5+）优先：它有 tab、能浮起，能力更全。
		 * 没有就退到 `details`（0.1.2-rc.1 这一版）。两条都没有才算真的不可用。
		 *
		 * 返回值里的 `channel` 是给调用方与测试分辨"走的哪条路"用的：两套的**读回判据
		 * 完全不同**（前者读 `active().id`，后者读 `ctx.layout` 之外没有可读状态，只能
		 * "调用没抛 + 槽位在渲染"），混成一句 `ok` 会让测不出差别。
		 */
		 ...
		 * 而「右侧栏」用 `details`。两者是**同一份内容的两种外壳**，不是两套实现。
		 */
		function rightColumnCapability(ctx) {
			var sidebarRight = rightbarService(ctx);
			if (sidebarRight !== null && typeof sidebarRight.openTab === "function") {
				return {
					channel: "sidebarRight",
					column: true,
					// 原生浮起要 `float()` 在；只有在的时候才把"悬浮"交给 DSH。
					nativeFloat: typeof sidebarRight.float === "function"
				};
			}
			var details = detailsChannel(ctx);
		...
		function switchLayout(place, appId, env) {
			...
			if (target.key === "drawer") {
				// 并列那一面走**右栏**：两条通道都认（`sidebarRight` 优先，退到 `details`）。
				// 先让它开出来、**读回确认**之后再写状态 —— "界面说切了、其实没切"是这一路
				// 反复出现的失败形态（幽灵座位、`float()` 静默 return 都是它）。
				//
