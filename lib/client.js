...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * 标题那一格。
		 *
		 * 官方配方（第一方 `packages/client/ui-sidebar-documentpreview/src/client/index.ts`）逐字如此：
		 * **类型进 `sidebarRightTabs`、正文进 `sidebar.right.pane.tab`、标题进
		 * `sidebar.right.pane.tab.title`** —— 两处登记用**同一把 key**（就是那个类型定义的 `id`）。
		 * 不登记标题也能开（页签显示登记时抓下来的 `title(address)`），但登记了才能在
		 * 我们的语言表切换时跟着走。
		 */
		var RIGHTBAR_TITLE_SLOT = "sidebar.right.pane.tab.title";
		/** 我们那个 tab 类型的 id / kind：`register({ id, kind })` 与座位 `key` 都用它。 */
		var RIGHTBAR_VIEW_ID = "dsh-miniapp";
		var RIGHTBAR_VIEW_KIND = "miniapp";
		/**
		 * 登记档位。官方 band 只有三档字符串（`ui-sidebar-right/src/client/tab-registry.ts`
		 * 的 `SidebarRightTabPriority`：`extension` / `builtin` / `fallback`），
		 * 缺省即 `extension`（"产品之外的实现"档，可以接管 builtin 的 kind）。
		 *
		 * **这里原来写的是 `0`** —— 0 不是合法 band：`RANKS[0]` 是 `undefined`，任何
		 * 档位比较都会得到 NaN（页面型用不到 claim 排序，所以一直没炸，但它是个假值）。
		 * 现在写成显式的 `extension`：我们是外部实现，这一档也保证将来要接管别人的 kind 时不用改。
		 */
		var RIGHTBAR_VIEW_PRIORITY = "extension";
		/** `+` 菜单（指南页）里的排序，DSH 的默认档是 100。 */
		var RIGHTBAR_GUIDE_ORDER = 100;
		/**
		 * 第三方侧栏插件 `dsh-better-sidebar` 提供的服务名（它 `ctx.provide('betterSidebar', service)`）。
		 * 只作 `ctx.get` 的键 —— **不进 `inject`**（理由见 `rightColumnCapability`）。
		 */
		var BETTER_SIDEBAR_SERVICE = "betterSidebar";

		// ------------------------------------------------ 这一版 DSH 的右栏：`details` 槽位
		//
		// **`sidebarRight` 在 0.1.2-rc.1 上不存在**（Inspect 的 client Service 目录里只有
		// layout / locale / sessions / slots / theme / timer / uiWorkspace / workspaces）。
		// 这个版本的右栏是**另一套**：槽位 `details`（kind single、scope session）+
		// `ctx.layout.openDetails()` / `closeDetails()`。DSH 自己的 ui-chat 就是这么用的
		// —— `ctx.slots.inject("details", …)` 里注册 DetailsPanel（dsh-client-ui-chat 的
		// `client.js`），它的 `inject` 回调里给 `closeDetails: () => ctx.layout.closeDetails()`。
		//
		// 所以"右栏打不开"从来不是这个 DSH 没有能力，是**我们找错了名字**：
		// 三条读回判定全挂在 `sidebarRight` 上，服务为 null 就立刻 no-service 退出，
		// 根本走不到 `details`。这一版把两套**都认**。
		var DETAILS_SLOT = "details";
		/**
		 * 遮蔽 `details` 用的优先级：**必须小于 DSH 自己的 0**。
		 *
		 * `kind: "single"` 的渲染判决是 `entriesOfSlot(key)[0]`，条目按 priority **升序**，
		 * 所以更小的赢（见 dsh-client-ui-renderer 的 `renderOutlet`）。DSH 的 ui-chat 用默认档
		 * 0 占着这一格；我们接过来就**必然顶掉工具详情面板** —— 这是这条路的固有代价，
		 * 不是我选了个激进的数：只要用 `details` 就一定会顶，差别只是谁赢。
		 * 负数是"我们赢、并且在谁赢这件事上不留给同档竞争"。
		 */
		var DETAILS_PRIORITY = -10;
		/** 记录我们是否真的遮蔽了 `details`（卸载时要按事实收回，而不是按意图）。 */
		var detailsShadowed = false;
		...
		var COPY = {
						zh: {
							...
							view: {
								...
								// 右栏指南（那本 `+` 菜单）里我们那一行下面的一句话。
								// DSH 只在指南条目不多时画 description；条目多时自动退回只画标题。
								rightbarDescription: "在这个会话的右栏里并排跑一个小程序。"
							},
							...
							bar: {
								...
								// 面板最下面那一条：整个小程序库的入口。
								manage: "管理小程序",
								// 分区标题只剩「全部小程序」：固定的那几个已经直接显示在标题栏
								// 胶囊的主段里，面板里再开一个固定区只会互相重复。
								sectionAll: "全部小程序",
								...
								pin: "固定到标题栏",
								unpin: "取消固定",
								// 固定满 8 个之后再点 📌：要说出来，而不是静默失败 ——
								// 静默的结局是用户连点三次、以为按钮坏了。
								pinLimit: "最多固定 {max} 个小程序，先取消一个再固定",
								// 胶囊主段放不下时的溢出标记（+k）的可访问名。
								morePinned: "还有 {count} 个固定的小程序",
								...
								open: {
									drawer: "在右侧打开",
									// 动词短语，与同块其他键一致（名词版是 `layout.place.corner`「右上浮窗」）。
									corner: "用悬浮窗打开",
									session: "在本会话页签打开"
								}
							},
							...
							open: {
								...
								missing: "这个小程序已经不在了。",
								// **只有并列那一面**要用 DSH 的原生右栏服务（`sidebarRight` / `details`），
								// 老宿主上两个都没有 —— 不能静默。悬浮不在此列：它本来就是自绘的
								// （`MiniAppFloatWindow` 自带几何、画在 `shell.overlay` 里），
								// 所以这句话**不许**再把悬浮一起说成打不开（那正是用户实测到的那次误报）。
								//
								// 这句话的判据是**服务一个都没有**（channel === "none"）。
								rightbarUnavailable: "这个 DSH 版本没有原生右栏服务，右侧栏（并列）暂时打不开。悬浮窗仍可用。",
								// 服务**在**、但读回确认不到我们那一格。与上一句分开写，是因为
								// 「没有服务」与「有服务但没开成」是两件事：前者换一个 DSH 版本才有救，
								// 后者可能是这一格被用户关掉了、或者第三方侧栏插件拒绝了这次打开。
								// 混成一句会让用户按错误的线索去排查（也正是在第三方通道上最容易误导的地方）。
								rightbarOpenFailed: "右侧栏没有确认打开我们那一格，并列这一面暂时没打开。悬浮窗仍可用。"
							},
							errors: {
						...
						en: {
							...
							view: {
								...
								rightbarDescription: "Run a mini-app side by side in this session's right column."
							},
							...
							bar: {
								...
								unpin: "Unpin",
								pinLimit: "Up to {max} mini-apps can be pinned — unpin one first",
								morePinned: "{count} more pinned mini-apps",
								...
								open: {
									...
									corner: "Open in the corner window",
							...
							open: {
								...
								// 与中文同一口径：只说**并列**打不开，并点明悬浮仍可用。
								rightbarUnavailable: "This DSH build has no native side-panel service, so the right column (docked) is unavailable for now. The floating window still works.",
								// A service IS there but the open did not read back: a different
								// failure from "no service at all" (see the zh comment).
								rightbarOpenFailed: "The right column did not confirm our pane opened, so the side-by-side view is unavailable for now. The floating window still works."
							},
							errors: {
		...
		var ICON_PATHS = {
			 ...
			 * 胶囊下拉段那枚 ▾（chevron 向下）。
			 *
			 * 与 `right`（chevron 向右）是同一支笔的两种朝向：主段的图标负责"打开",
			 * 下拉段负责"展开面板"，朝向就是这两件事的分界 —— ▾ = "这里还能展开一层"。
			 */
			chevronDown: "M6 9.4 12 15.4 18 9.4 16.6 8 12 12.6 7.4 8z",
		 ...
		 * 标题栏可固定的小程序数量上限。
		 *
		 * 与宿主半边的 `PREFS_PINNED_MAX`（lib/index.js）必须**相等**：客户端在点 📌
		 * 的那一刻就用它拦住"第 9 个"，宿主在落盘那一步再拦一次 —— 两道闸各写一份
		 * 常量是 wire 契约两边各写一份的既成格局，相等由测试钉住（见两条
		 * `PINNED_MAX` 的断言），不靠人记。为什么是 8 见宿主那边的注释。
		 */
		var PINNED_MAX = 8;

		/**
		 * `last_place_by_app` 的值白名单：`LAYOUT_PLACES` 里除 `browser` 外的四个键。
		 *
		 * 与宿主半边的 `LAST_PLACE_KEYS` 一一对应（同样由测试钉住）。browser 刻意
		 * 不在名单里：它开的是 DSH 之外的新页签，不是"呈现面"（理由详见宿主侧注释）。
		 */
		var LAST_PLACE_KEYS = ["panel", "drawer", "session", "corner"];

		/**
		 * 从 `/prefs` 的响应里取**新形状**（`pinned_app_ids` 数组 + `last_place_by_app`
		 * 映射）；形状不对的条目一律丢弃，绝不抛 —— 旧宿主 / 半新不旧的盘上状态都要
		 * 能开成客户端这一份。
		 *
		 * 读端点是宿主 `normalisePrefsShape` 的镜像：那边保证过形状，这边再兜一层
		 * （端点之间还隔着一次网络与一个 JSON.parse，客户端不能假设对面永远完美）。
		 */
		function readPrefsShape(data) {
			var out = { pinnedAppIds: [], lastPlaceByApp: {} };
			if (data === null || data === undefined || typeof data !== "object") return out;
			var ids = data.pinned_app_ids;
			if (Array.isArray(ids)) {
				for (var index = 0; index < ids.length; index += 1) {
					var id = ids[index];
					if (typeof id !== "string" || id === "") continue;
					if (out.pinnedAppIds.indexOf(id) >= 0) continue;
					if (out.pinnedAppIds.length >= PINNED_MAX) break;
					out.pinnedAppIds.push(id);
				}
			}
			var places = data.last_place_by_app;
			if (places !== null && typeof places === "object" && !Array.isArray(places)) {
				for (var key in places) {
					if (Object.prototype.hasOwnProperty.call(places, key) !== true) continue;
					if (typeof key !== "string" || key === "") continue;
					var value = places[key];
					if (LAST_PLACE_KEYS.indexOf(value) < 0) continue;
					out.lastPlaceByApp[key] = value;
				}
			}
			return out;
		}
		...
		/**
		 * 点一次 📌 之后固定的应该是哪些 —— **多选**语义的全部判决：
		 *
		 *  * `appId` 已经在固定列表里 → 把它**移除**（其余保持原顺序）；
		 *  * 不在 → **追加**到末尾（不是顶掉谁 —— 固定可以有多个）；
		 *  * 不在、但已经固定满 `max` 个 → 返回 `null`：这一下没有生效，调用方
		 *    必须把"满了"说出来，而不是静默吞掉。
		 *
		 * 抽成纯函数而不是写在 `pin` 里：这条判决是"再点一次会怎样"的全部答案，
		 * 而它错了的表现（点一次固定、再点一次没取消、或者悄悄固定了 9 个）只有
		 * 真的点两遍才看得出来。
		 *
		 * 两个容错：
		 *  * `previous` 是坏的（不是数组）一律当成"什么都没固定"：磁盘上一条脏数据
		 *    不该被当成"固定了一个叫 undefined 的小程序"；
		 *  * `appId` 是坏的（不是字符串、或空串）**原样返回**清理过的列表 ——
		 *    一个认不出来的 id 不该被理解成"取消固定全部"。
		 */
		function togglePinned(previous, appId, max) {
			var limit = typeof max === "number" && Number.isFinite(max) && max > 0 ? max : PINNED_MAX;
			var list = [];
			if (Array.isArray(previous)) {
				for (var index = 0; index < previous.length; index += 1) {
					var id = previous[index];
					if (typeof id !== "string" || id === "") continue;
					if (list.indexOf(id) >= 0) continue;
					list.push(id);
				}
			}
			if (typeof appId !== "string" || appId === "") return list;
		...
		function MiniAppPrefs() {
			this.state = { pinnedAppIds: [], lastPlaceByApp: {}, notice: null, loaded: false, error: null };
			this.listeners = new Set();
			this.inflight = null;
			// 写请求的串行队列：pin 与 recordPlace 用的都是**全量替换**的 POST body，
			// 并发在飞时会互相覆盖（后落盘的那个带着旧的那半边），串起来才有"每次写的
			// 都是当时最新的一份"这个保证。队列永不停摆：失败的链节直接跳过。
		 ...
		 * 读一次。
		 *
		 * 失败**不弹错**，只是当成"没有偏好"：它影响的只有标题栏上那排图标与"下次
		 * 打开去哪儿"，没有任何理由为它弹一条错误、更没有理由挡住会话头部。`loaded`
		 * 照样置真 —— 否则一次失败会让"没读到过"永远成立，每次渲染都重打一次后端
		 * （错误留在 `state.error` 上，需要重读的人看它，见 `ensureReadable`）。
		 */
		MiniAppPrefs.prototype.load = function (force) {
			...
			this.inflight = callApi("/prefs").then(function (data) {
				self.inflight = null;
				var shape = readPrefsShape(data);
				self.set({ pinnedAppIds: shape.pinnedAppIds, lastPlaceByApp: shape.lastPlaceByApp, loaded: true, error: null });
				return self.state;
			}, function (error) {
				self.inflight = null;
		...
		/**
		 * 写之前的可读性检查：state 里得有一份**可信的**盘上现状，否则一次全量 POST
		 * 会把没读到的那半边（固定列表 / 上次呈现面）在盘上清空。
		 *
		 * 什么时候 state 不可信：还没 load 过，或者上次 load 失败过（`error` 非空 ——
		 * 那时 `pinnedAppIds` / `lastPlaceByApp` 停留在默认的空值上）。两种情况都先
		 * 强制重读一次；重读仍然失败就**放弃这次写**（返回 false），调用方各自决定
		 * 怎么收场 —— 但绝不能拿着空 state 去覆盖盘上可能有东西的那份。
		 */
		MiniAppPrefs.prototype.ensureReadable = async function () {
			if (this.state.loaded === true && this.state.error === null) return true;
			await this.load(true);
			return this.state.error === null;
		};

		/** 把当前 state 组装成 POST body（wire 契约的两半边都在，缺一不可）。 */
		MiniAppPrefs.prototype.postBody = function () {
			return {
				pinned_app_ids: this.state.pinnedAppIds.slice(),
				last_place_by_app: Object.assign({}, this.state.lastPlaceByApp)
			};
		};

		/** 真正打 POST（挂在串行队列上）；失败时把原因留在 state.error 上（不抛）。 */
		MiniAppPrefs.prototype.postPrefs = function () {
			var self = this;
			var send = this.queue.then(function () {
				return callApi("/prefs", { method: "POST", body: self.postBody() });
			});
			this.queue = send.then(function () { return undefined; }, function () { return undefined; });
			return send;
		};
		 ...
		 *
		 * 满了（`togglePinned` 返回 null）**不写盘**，只把一条 `notice` 放进 state：
		 * 面板会把它画出来。静默失败是这个动作最坏的收场（用户连点三次、以为按钮坏了）。
		 * notice 不自动过期：下一次**成功的**固定/取消会把它清掉，在那之前提示一直
		 * 有效（不引入定时器 —— 测试替身的 setTimeout 是 no-op，真机上它也只是锦上添花）。
		 *
		...
		MiniAppPrefs.prototype.pin = function (appId) {
			...
			var run = async function () {
				// 顺序是刻意的，动它之前先读这段：
				//  1. **乐观更新必须先行**（那颗图标要跟着手指走）—— 正常路径上
				//     （已读过 prefs）下面没有任何 await 在 set 之前，同步就落下；
				//  2. state 不可信（还没读过 / 上次读失败）时**先补读再动笔**：POST 是
				//     全量替换，拿着空 state 发出去会把盘上那半边清空。补读发生在
				//     乐观更新**之前** —— 补读会重置本地列表，先更新再补读等于白更新。
				var next = togglePinned(self.state.pinnedAppIds, appId);
				if (next === null) {
					// 满了：只提示，不动盘、不动列表。
					self.set({ notice: "pin-limit" });
					return;
				}
				if (self.state.loaded !== true || self.state.error !== null) {
					if (!(await self.ensureReadable())) return;
					next = togglePinned(self.state.pinnedAppIds, appId);
					if (next === null) {
						self.set({ notice: "pin-limit" });
						return;
					}
				}
				var previous = self.state.pinnedAppIds;
				var previousNotice = self.state.notice;
				self.set({ pinnedAppIds: next, notice: null, error: null });
				try {
					await self.postPrefs();
				} catch (error) {
					self.set({ pinnedAppIds: previous, notice: previousNotice, error: describeError(error) });
				}
			};
			return run().catch(function () { return undefined; });
		};

		/**
		 * 记录"这个小程序上次被摆进了哪个呈现面"（`last_place_by_app` 的写入口）。
		 *
		 * 调用方是 `switchLayout` 的成功路径：切换已经发生，这里的写入**绝不阻塞它** ——
		 * 失败静默回滚（本地回到写之前那一份，与 `pin` 的失败处理同构：本地状态必须
		 * 与盘上一致，否则"下次打开去哪儿"会在重启前后悄悄变卦）。
		 *
		 * 值没变就不写（同一个面里反复刷新不打扰盘）；白名单外的 place（含 browser）
		 * 直接忽略 —— 这个字段只记录**呈现面**。
		 */
		MiniAppPrefs.prototype.recordPlace = function (appId, place) {
			var self = this;
			var run = async function () {
				if (typeof appId !== "string" || appId === "") return;
				if (LAST_PLACE_KEYS.indexOf(place) < 0) return;
				if (!(await self.ensureReadable())) return;
				var previous = self.state.lastPlaceByApp;
				if (previous !== null && typeof previous === "object" && previous[appId] === place) return;
				var next = Object.assign({}, previous);
				next[appId] = place;
				self.set({ lastPlaceByApp: next, error: null });
				try {
					await self.postPrefs();
				} catch (error) {
					self.set({ lastPlaceByApp: previous, error: describeError(error) });
				}
			};
			return run().catch(function () { return undefined; });
		};
		...
		// ------------------------------------------- 第三方通道：`dsh-better-sidebar`
		//
		// 它自己 `ctx.provide('betterSidebar', service)`（`src/client/index.tsx:143`），
		// 并把**每一个** `registerTab` 的描述符镜像成 DSH 原生 tab 类型：`src/client/native/index.ts`
		// 里 `tabs.register({ id: 'dsh-better-sidebar:' + descriptor.id, kind: descriptor.id, … })`。
		// 也就是说：**登记到它那里，就等于同时登记进了 DSH 的原生右栏**，
		// 而它的 `openTab` 默认落点就是右栏（`OpenTabSeed.target` 的注释：`'right'` 是默认）。
		//
		// 探测一律走 `ctx.get`（可选服务），**绝不写进 `inject`**：`inject` 是硬依赖，
		// 缺服务的构建上整个客户端半边会被挂起等待（本文件 `currentSessionId` 上面那段
		// 已经记过这条教训；DSH 的 packages/client/AGENTS.md 也把 `ctx.get` 定为可选服务的读法）。

		/** 取第三方侧栏服务：拿不到返回 null。 */
		function betterSidebarService(ctx) { return serviceOf(ctx, BETTER_SIDEBAR_SERVICE); }

		/**
		 * `betterSidebar` 通道 —— **注册面与打开面都在**才算可用。
		 *
		 * 两件事分开判：只有 `getTabs`/`subscribe`（只读面）不能登记，只有 `registerTab`
		 * 打不开。少任何一样，这条通道对我们就是"看着在、其实做不成事"。
		 */
		function betterSidebarChannel(ctx) {
			var service = betterSidebarService(ctx);
			if (service === null) return null;
			if (typeof service.registerTab !== "function") return null;
			if (typeof service.openTab !== "function") return null;
			return { channel: "betterSidebar", service: service };
		}

		/**
		 * `betterSidebar` 通道的**读回凭据**。
		 *
		 * 它那两个方法同样**没有返回值**（`openTab(seed): void`、`registerTab(desc): disposer`），
		 * 所以"没抛错"同样不算数。它给出的**真信号**是描述符自己的生命周期回调：
		 * `onOpen`（真的开了一格）与 `onActivate`（去重聚焦已有那一格）—— `service.ts` 里
		 * **只有**在 `isTabEnabled` / 描述符存在 / 拿到 sessionId / `createTab` 没拒绝
		 * 这几道门全过了之后才回调（那是"这次打开真的落地了"的证据），
		 * 而这个对象就是那两个回调的计数落点。
		 *
		 * 它是模块级的，因为登记（apply 里）与打开（`switchLayout` 里）不在同一个作用域；
		 * 登记时写入、`registerBetterSidebarTab` 的 disposer 里清掉（`!== pulse` 的比较保证
		 * 后一次登记不会把前一次的清理误当成自己那次）。
		 */
		var betterSidebarPulse = null;

		/**
		 * 把「小程序」登记成 `betterSidebar` 的一个 tab 类型，返回可回收的 disposer。
		 *
		 * 三处要点：
		 *   * `id` 用 **`RIGHTBAR_VIEW_KIND`**（"miniapp"）而不是 `RIGHTBAR_VIEW_ID`：
		 *     它隔壁会把这个 id 原样当成**原生 kind**（`kind: descriptor.id`），
		 *     与"我们自己在原生通道登记时用的 kind"逐字相同 —— 于是原生那两扇门
		 *     （页签要按 kind 找类型、正文要按实现 id 找）在两条路上指向同一格。
		 *   * `component` 复用**同一份** `MiniAppRightbarPane`（"一份 iframe 实现"这条原则）；
		 *     这里只是给它一颗自己的 `t`/`ctx`/`ui` 外壳，没有第二份运行体。
		 *   * `title`/`description` 写成**返回函数**：它的 `+` 菜单与设置页在每次渲染时求值，
		 *     切语言才会跟着走（字符串会被冻结在登记那一刻）。
		 */
		function registerBetterSidebarTab(options) {
			var service = options.service;
			var pulse = { service: service, opened: 0, activated: 0 };
			var descriptor = {
				id: RIGHTBAR_VIEW_KIND,
				title: function () { return options.t("view.tab"); },
				description: function () { return options.t("view.rightbarDescription"); },
				// 一个纯字形的图标（它的 TabDescriptor.icon 接受 `(size) => ReactNode`）；
				// 不用 SVG 是为了不给这一格引入任何样式依赖。
				icon: function (size) {
					return React.createElement("span", {
						style: { fontSize: (typeof size === "number" ? size : 16) + "px", lineHeight: 1 }
					}, "🧩");
				},
				order: RIGHTBAR_GUIDE_ORDER,
				// 我们这一面在每个会话里只有一格（`openTab` 落到同一格就是聚焦）。
				single: true,
				onOpen: function () { pulse.opened += 1; },
				onActivate: function () { pulse.activated += 1; },
				component: function () {
					return React.createElement(MiniAppRightbarPane, { t: options.t, ctx: options.ctx, ui: options.ui });
				}
			};
			var dispose = service.registerTab(descriptor);
			betterSidebarPulse = pulse;
			return function () {
				if (betterSidebarPulse === pulse) betterSidebarPulse = null;
				if (typeof dispose === "function") dispose();
			};
		}

		/**
		 * 原生通道的类型定义（`ctx.sidebarRightTabs.register` 的那一份）。
		 *
		 * 逐字对照 `ui-sidebar-right/src/client/tab-registry.ts:87` 的
		 * `SidebarRightTabDefinition`：
		 *   * `id` —— 实现身份，**正文与标题座位就是按它找 body 的**（契约注释逐字如此）；
		 *   * `kind` —— `openTab(kind)` 认的那个名字；
		 *   * `title(address)` —— **必填**，打开时被抓进 tab 记录当页签文字。原来这里漏了它，
		 *     于是 `placeTab` 里 `definition.title(address)` 会抛 TypeError（打开必然失败）；
		 *   * `priority` —— 三档字符串（见 `RIGHTBAR_VIEW_PRIORITY`）；
		 *   * **`patterns` 不填** —— 那是"这个类型认领哪些资源地址"（`dsh-resource://…`），
		 *     我们是**页面型**，由 kind 打开、不认领任何地址。填了反而会去参与地址路由。
		 *   * `guide` —— 指南页（`+` 菜单点开的那一页）里我们那一行；没有它，
		 *     用户从右栏里根本找不到「小程序」这个入口。
		 */
		function nativeRightbarDefinition(t) {
			return {
				id: RIGHTBAR_VIEW_ID,
				kind: RIGHTBAR_VIEW_KIND,
				priority: RIGHTBAR_VIEW_PRIORITY,
				title: function () { return t("view.tab"); },
				guide: [{
					order: RIGHTBAR_GUIDE_ORDER,
					title: function () { return t("view.tab"); },
					description: function () { return t("view.rightbarDescription"); }
				}]
			};
		}

		/**
		 * 通过第三方通道把「小程序」开到右栏（`openTab` 的默认落点就是 `'right'`）。
		 *
		 * 读回判定分两问，缺一不可：
		 *   ① 它的生命周期回调动了吗？（没动 = 这次打开被它拒了：类型被用户在它的设置里
		 *      关掉、描述符不在、或者拿不到 sessionId —— 三种都是"什么都没开"）；
		 *   ② 原生右栏服务在吗？**这一问是必须的**：它的 native surface 在拿不到
		 *      `ctx.sidebarRight` 时会把这次打开塞进自己的 pending 队列并返回 false，
		 *      却**照样**回调 `onOpen`（`src/client/native/surface.ts:63-98` + `service.ts` 的
		 *      `safeCall(onOpen)`）。只看 ① 就会把"排队等着"当成"已经开了"。
		 */
		function openBetterSidebarPane(ctx) {
			var channel = betterSidebarChannel(ctx);
			if (channel === null) return { ok: false, reason: "no-service", observed: null };
			var pulse = betterSidebarPulse;
			if (pulse === null || pulse.service !== channel.service) {
				return { ok: false, reason: "not-registered", observed: null };
			}
			var before = pulse.opened + pulse.activated;
			try {
				channel.service.openTab({ type: RIGHTBAR_VIEW_KIND });
			} catch (error) {
				return { ok: false, reason: "threw", observed: null };
			}
			var observed = { opened: pulse.opened, activated: pulse.activated, native: observeRightbar(rightbarService(ctx)) };
			if (pulse.opened + pulse.activated <= before) return { ok: false, reason: "not-observed", observed: observed };
			if (rightbarService(ctx) === null) return { ok: false, reason: "no-native-column", observed: observed };
			return { ok: true, reason: "opened", observed: observed };
		}

		/**
		 * 这一版 DSH 的**另一个**右栏通道：`details` 槽位 + `ctx.layout` 的两扇门。
		 *
		 * 两个能力分开探测，因为它们**可以独立缺失**：DSH 有 `layout.openDetails` 但
		 * 没有 `details` 槽位（或反过来）时，只有"两样都在"才算这条路可用 —— 少了任何一样
		 * 都是"开了也看不见"，那种"以为成功了"正是这一路反复出现的失败形态。
		 */
		function layoutService(ctx) { return serviceOf(ctx, "layout"); }

		/**
		 * 探测 `details` 通道是否可用，返回一个**自己的**能力对象（不把 ctx 交出去）。
		 *
		 * 为什么返回对象而不是布尔：三处调用点要的都是同一组动作（open / close / 读回），
		 * 让它们各自去 `ctx.get` 三遍只会写歪一处。对象里的方法全部**只收自己**闭包里的
		 * service，形状稳定、可被测试替身替换。
		 */
		function detailsChannel(ctx) {
			var layout = layoutService(ctx);
			var canOpen = layout !== null && typeof layout.openDetails === "function";
			var canClose = layout !== null && typeof layout.closeDetails === "function";
			if (!canOpen && !canClose) return null;
			return {
				channel: "details",
				open: function () {
					if (!canOpen) return false;
					try { layout.openDetails(); return true; } catch (error) { return false; }
				},
				close: function () {
					if (!canClose) return false;
					try { layout.closeDetails(); return true; } catch (error) { return false; }
				}
			};
		}

		/**
		 * 打开右栏 —— **三条通道都认**，返回我们实际用上的那一条。
		 *
		 * 优先级（每条都有理由，不是先来后到）：
		 *
		 *   ① `betterSidebar`（第三方 `dsh-better-sidebar`）**优先**。理由不是"它更好"，
		 *      而是**它很可能正掌管着用户的侧栏 UI**：它自己那只 Sidebar 自绘了页签与
		 *      `+` 菜单（`src/client/Sidebar.tsx` / `add-plugin-modal.tsx`），
		 *      并把登记进来的描述符镜像成原生 tab 类型。用户看到的"+"是它渲染的 ⇒
		 *      我们只在它那里登记一格，用户就能在**他实际看的那个菜单**里找到「小程序」；
		 *      反过来（只在原生登记）在它接管 UI 的构建上等于把入口藏起来。
		 *   ② `sidebarRight`（DSH 0.1.5+ 的原生右栏）：有 tab、能浮起，能力最全。
		 *   ③ `details`（0.1.2-rc.1 那一版）：槽位 + `ctx.layout.openDetails()`。
		 *
		 * **①在时不回落**：回落到原生会绕开它自己的设置（用户可能在它的设置里把这一格
		 * 关掉了），还可能开出第二份内容/第二个同名菜单条目。它说没开成，就如实失败。
		 *
		 * 返回值里的 `channel` 是给调用方与测试分辨"走的哪条路"用的：三条的**读回判据
		 * 完全不同**，混成一句 `ok` 会让测不出差别。
		 *
		 * `dispatched`（只有 `sidebarRight` 那一路会给）把"动作发出去了"与"读回确认了"
		 * 分开说：`switchLayout` 的 drawer 分支据此把"有通道、只是同一 tick 读不到"
		 * 交给**有界读回**，而不是当场判失败（见 `scheduleRightbarOpenConfirm`）。
		 */
		function openRightColumn(ctx) {
			var better = betterSidebarChannel(ctx);
			if (better !== null) return openBetterSidebarPane(ctx);
			var sidebarRight = rightbarService(ctx);
			if (sidebarRight !== null && typeof sidebarRight.openTab === "function") {
				var step = openRightbarPane(ctx);
				return {
					ok: step.ok, channel: "sidebarRight", reason: step.reason,
					observed: step.observed, dispatched: step.dispatched === true
				};
			}
			var details = detailsChannel(ctx);
			if (details !== null) {
				var opened = details.open();
				return { ok: opened, channel: "details", reason: opened ? "opened" : "threw", observed: null };
			}
			return { ok: false, channel: "none", reason: "no-service", observed: null };
		}

		/**
		 * 关闭右栏 —— 三条通道都认；返回是否真的发出了一次关闭。
		 *
		 * 第三方那一路要**先读回真正那一格的 TabId**再交给它 `closeTab`：
		 * DSH 铸造的 tab id 是 `tab<n>`（`ui-dockkit/src/engine/initial.ts:27-29`），
		 * 不是我们的类型 id —— 拿类型 id 去关是关不掉的（而且不抛错，属于静默失败）。
		 * 读不到 id 就如实 false，不猜一个。
		 */
		function closeRightColumn(ctx) {
			var better = betterSidebarChannel(ctx);
			if (better !== null) {
				if (typeof better.service.closeTab !== "function") return { ok: false, channel: "betterSidebar" };
				var active = observeRightbar(rightbarService(ctx));
				if (active.docked !== true || active.activeId === null) return { ok: false, channel: "betterSidebar" };
				try {
					better.service.closeTab(active.activeId);
					return { ok: true, channel: "betterSidebar" };
				} catch (error) {
					return { ok: false, channel: "betterSidebar" };
				}
			}
			var sidebarRight = rightbarService(ctx);
			if (sidebarRight !== null && typeof sidebarRight.close === "function") {
				try { sidebarRight.close(RIGHTBAR_VIEW_ID); return { ok: true, channel: "sidebarRight" }; } catch (error) { return { ok: false, channel: "sidebarRight" }; }
			}
			var details = detailsChannel(ctx);
			if (details !== null) return { ok: details.close(), channel: "details" };
			return { ok: false, channel: "none" };
		}

		// ------------------------------------------------ 「并列」打开的有界读回
		//
		// 真机症状（2026-10，用户实测）：点「并列」弹出的是 `open.rightbarOpenFailed`。
		// 那句话的**唯一**触发点是"有通道、但读回没确认" —— 也就是 `sidebarRight`
		// 是可达的，只是**同一 tick 读不到**我们那一格。
		//
		// 根因（对着用户机器上装的那份 asar 逐字核过）：
		//   * `sidebarRight.openTab(kind)` 只做一件事：把这次打开写成一次 store 意图
		//     （`node_modules/@deepseek-ai/dsh-client-ui-sidebar-right/lib/client.js` 里
		//     `openTab(kind, opts)` → `placeTab(...)` → `place(...)` → `actions.openContent({
		//     kind, contentId, title, … })`）；
		//   * 而 `active()` 读的是 **`this.binding.surfaces[this.binding.sessionId]`**
		//     （同文件 `mounted()`；`active()` 再从它取 layout），`binding` 由挂载中的
		//     `RightbarSeat` 在**被动 effect** 里 `bindService(...)` 写入，依赖表里带着
		//     `surfaces`（store 快照）；
		//   * store 一变就是一次 React 提交 + 一次 effect，**在 `openTab` 返回时还没发生**
		//     ⇒ 紧随其后的 `active()` 读到的必然是**打开前**的 layout，一次已经发出、
		//     正在路上的打开被判成 `not-present`。
		//
		// 所以给它一个有界读回窗口：**先同步读一次**（快路径：DSH 同步提交、或那一格
		// 本来就是我们在前台时它当场成立 —— 不引入任何无条件延迟），没读到才重读。
		//
		// 参数与理由（N / 间隔 / 总时长）：
		//   * 重读 **8** 次、每次间隔 **50ms** ⇒ 窗口 **≤ 400ms**；
		//   * 50ms ≈ 3 帧（60fps 下 16.7ms/帧）：上面那条被动 effect 通常在一帧内被刷掉，
		//     50ms 是给"先把列展开、再挂正文"那一串留的余量；
		//   * 400ms 比它需要的多两个数量级，又**远小于 1s 的上限**、短于 toast 的 6s ——
		//     用户在"点了没反应"之前就拿到结论（成功，或一句如实的失败）；
		//   * 8 次是**硬上限**：窗口用尽即判失败，**绝不无限重试**，也绝不把
		//     "读不到"（null = 未知）当成功。
		var RIGHTBAR_OPEN_CONFIRM_ATTEMPTS = 8;
		var RIGHTBAR_OPEN_CONFIRM_INTERVAL_MS = 50;

		/**
		 * 当前那条"发出去了、还没读回"的并列打开。任何时刻**最多一条** ——
		 * 新的打开会把上一条撤掉（旧结论已经没有资格再写状态了）。
		 */
		var pendingRightbarOpen = null;

		/**
		 * 每一次**被接受的** `switchLayout` 都会 +1。
		 *
		 * 唯一的用途是给**迟到的**读回回调一个判据：序号变了就说明"用户后来已经选了
		 * 别的地方 / 别的小程序"，那次迟到的结论不许再写状态（见
		 * `drawerIntentStillCurrent`）。它是模块级的，因为判的是"最后一次动作是谁做的"，
		 * 与 React 什么时候渲染无关。
		 */
		var layoutActionSeq = 0;

		/** 现在有没有一条并列打开在等读回（`MiniAppOverlaySeat` 的清理要用它）。 */
		function rightbarOpenPending() {
			return pendingRightbarOpen !== null;
		}

		/** 撤掉当前那条待确认的打开：清定时器、作废结论。返回是否撤掉了一条。 */
		function cancelPendingRightbarOpen() {
			var record = pendingRightbarOpen;
		 ...
		 * 把一次重读排到"稍后"。
		 *
		 * 首选 `window.setTimeout`：真机上就是它 —— 间隔可控、句柄可清（可回收）。
		 * 没有它时（只可能在没有浏览器的沙箱里）退到**微任务**：重读这件事不该因此
		 * 消失，否则调用方那句"动作已经发出去了"就永远等不到结论。微任务没有可清的
		 * 句柄，只能靠调用方的 `cancelled` 标记作废 —— 好在它同样只跑 N 次就停，
		 * 有界性不变。
		 *
		 * @returns 取消函数；**一个都排不上**（既没有 setTimeout 也没有 Promise）时
		 *          返回 `null`，调用方据此退回旧的严格语义。
		 */
		function scheduleRightbarRecheck(run, delay) {
			if (typeof window !== "undefined" && typeof window.setTimeout === "function") {
				var handle = window.setTimeout(run, delay);
				return function () {
			...
			if (typeof Promise === "function") {
				...
				return function () { /* 微任务没有句柄：作废靠 run 里的 cancelled 标记 */ };
			}
			return null;
		}

		/**
		 * 为一次"已经发出、同步还没读到"的并列打开安排**有界读回**。
		 *
		 * 契约：
		 *   * 这里只重读、**不写状态** —— 结论通过 `settle(observed)` 交回调用方；
		 *     "这次还算不算数"（竞态）与"写什么状态"都在调用方那一处，不会写一半；
		 *   * 到点**只回调一次**，随后 `pendingRightbarOpen` 清空；
		 *   * 取消函数由调用方持有（新的打开、`switchLayout` 的入口、以及插件停用时的
		 *     `ctx.effect` disposer 都会用它），保证"已经没人要的验证"不会在后来某个
		 *     时刻写 ui；
		 *   * 返回 `false` = 连一次重读都排不上（既没有 setTimeout 也没有 Promise），
		 *     调用方必须按**旧的严格语义**处理（同步读回是唯一判据，如实失败）。
		 */
		function scheduleRightbarOpenConfirm(sidebarRight, settle) {
			cancelPendingRightbarOpen();
			var attempt = 0;
			var cancelled = false;
			var cancelRecheck = null;
			var record = {
				cancel: function () {
					cancelled = true;
					if (cancelRecheck !== null) cancelRecheck();
					cancelRecheck = null;
					settle = null;
				}
			};
			var finish = function (observed) {
				if (pendingRightbarOpen === record) pendingRightbarOpen = null;
				var callback = settle;
				settle = null;
				if (callback !== null) callback(observed);
			};
			var step = function () {
				if (cancelled) return;
				cancelRecheck = null;
				...
				// 读到了我们那一格、或者窗口已经用尽 —— 两种都是**结论**，只下一次。
				if (observed.docked === true || attempt >= RIGHTBAR_OPEN_CONFIRM_ATTEMPTS) {
					finish(observed);
					return;
				}
				cancelRecheck = scheduleRightbarRecheck(step, RIGHTBAR_OPEN_CONFIRM_INTERVAL_MS);
				if (cancelRecheck === null) finish(observed);
			};
			pendingRightbarOpen = record;
			cancelRecheck = scheduleRightbarRecheck(step, RIGHTBAR_OPEN_CONFIRM_INTERVAL_MS);
			if (cancelRecheck === null) {
				if (pendingRightbarOpen === record) pendingRightbarOpen = null;
				cancelled = true;
				return false;
			}
		...
		/**
		 * 迟到的读回还有资格写状态吗 —— "用户后来另作选择"的判据，任一成立即放弃。
		 *
		 *   * **动作序号变了**：这期间又发生过一次被接受的 `switchLayout`（换了面、
		 *     或换了小程序）—— 那是用户**后来**的选择，不能被这次迟到的成功覆盖；
		 *   * ui 现在明确指着**另一个**小程序（`appId` 非空且不是我们要开的那个）；
		 *   * ui 的呈现面从我安排时的那一个变成了别的（用户切走了，或者把面关了）。
		 *
		 * 为什么**不**比较整份 ui 快照：`MiniAppOverlay` 会把 `runningId` 那条一次性
		 * 命令在挂载与每次 ui 变化时收走（`ui.set({ runningId: null })`）。拿整份快照
		 * 当判据，一次与"用户去哪儿"无关的内部写入就会把这次打开判成"被取代" ——
		 * 结果是用户点了并列却既不成功也不失败（什么都不发生）。所以只比上面三条
		 * **语义**判据。
		 */
		function drawerIntentStillCurrent(actionSeq, appId, modeAtDispatch) {
			if (layoutActionSeq !== actionSeq) return false;
			var state = ui.get();
			if (state.appId !== null && state.appId !== appId) return false;
			if (state.mode !== modeAtDispatch) return false;
			return true;
		}

		/**
		 * 右栏能力面 —— 给 UI 用的一句真话：哪条通道在、能不能浮起。
		 *
		 * 「能不能浮起」只有 `sidebarRight` 有（`float()`）；`details` 槽位**只有开/关两态**，
		 * 没有浮起。所以这一版上「悬浮」是**我们自己画窗口**（见 `MiniAppFloatWindow`），
		 * 而「右侧栏」用原生/第三方通道。两者是**同一份内容的两种外壳**，不是两套实现。
		 *
		 * `nativeFloat` 的判据**不因为第三方通道在就改**：浮起是 DSH 那一列自己的动作
		 * （`float(tabId)`），与"谁登记这个 tab 类型"无关；只要 `float()` 在，
		 * 悬浮就该交给 DSH 画，`MiniAppOverlaySeat` 才不会又画一个自绘窗口。
		 */
		function rightColumnCapability(ctx) {
			var sidebarRight = rightbarService(ctx);
			var nativeOpen = sidebarRight !== null && typeof sidebarRight.openTab === "function";
			var nativeFloat = nativeOpen && typeof sidebarRight.float === "function";
			if (betterSidebarChannel(ctx) !== null) {
				return { channel: "betterSidebar", column: true, nativeFloat: nativeFloat };
			}
			if (nativeOpen) return { channel: "sidebarRight", column: true, nativeFloat: nativeFloat };
			var details = detailsChannel(ctx);
			if (details !== null) return { channel: "details", column: true, nativeFloat: false };
			return { channel: "none", column: false, nativeFloat: false };
		}
		...
		function observeRightbar(sidebarRight) {
			var unknown = { docked: null, floating: null, expanded: null, activeId: null, activeKind: null, activeContentId: null };
			...
			if (typeof active === "string") activeId = active;
				...
				// `TabRecord` 的形状是 `{ id, kind, contentId, title }`
				// （`ui-dockkit/src/contract/types.ts:93-98`），三者都读出来。
				if (typeof active.kind === "string") activeKind = active.kind;
				if (typeof active.contentId === "string") activeContentId = active.contentId;
			}
			var expanded = null;
			try {
			...
			// "停靠" = 活动 tab 就是我们的那一个。读不到活动 tab 就是**未知**（null），不是 false。
			//
			// **为什么三个字段都认**：`id` 是 DSH 铸造的 `tab<n>`
			// （`ui-dockkit/src/engine/initial.ts:27-29`），**不是**我们的类型 id ——
			// 只比 `id` 的话，真机上永远读不到"停靠"（一次假失败）。`kind` 是我们打开的
			// 那个类型（`openTab(kind)`），`contentId` 是页面地址 `sidebar://<kind>`
			// （`ui-sidebar-right/src/client/contract/seed.ts` 的 `pageAddress`）——
			// 后两个才是"这一格是不是我们"的稳定判据。
			var docked = null;
			if (activeId === null && activeKind === null && activeContentId === null) docked = null;
			else {
				docked = activeId === RIGHTBAR_VIEW_ID
					|| activeKind === RIGHTBAR_VIEW_KIND
					|| activeKind === RIGHTBAR_VIEW_ID
					|| activeContentId === "sidebar://" + RIGHTBAR_VIEW_KIND
					|| activeContentId === "sidebar://" + RIGHTBAR_VIEW_ID;
			}
			...
			return {
				docked: docked, floating: floating, expanded: expanded,
				activeId: activeId, activeKind: activeKind, activeContentId: activeContentId
			};
		}
		 ...
		 *
		 * `dispatched` 与 `ok` 是**两件事**：前者 = "这次调用真的发出去了"（没抛错），
		 * 后者 = "同步读回确认它落地了"。分开的理由在 `switchLayout` 的 drawer 分支：
		 * 同一 tick 读不到并不等于没开成 —— 真机上那条失败**全部**长这样。
		 * 既有调用点与断言只读 `ok` / `reason` / `observed`，所以这是纯加法。
		 */
		function openRightbarPane(ctx) {
			var sidebarRight = rightbarService(ctx);
			if (sidebarRight === null || typeof sidebarRight.openTab !== "function") {
				return { ok: false, reason: "no-service", observed: null, dispatched: false };
			}
			try {
				sidebarRight.openTab(RIGHTBAR_VIEW_KIND);
			} catch (error) {
				return { ok: false, reason: "threw", observed: null, dispatched: false };
			}
			var observed = observeRightbar(sidebarRight);
			var step = presentationStep("open", observed);
			return { ok: step.ok, reason: step.reason, observed: observed, dispatched: true };
		}
		 ...
		 *
		 * 「并列」那一面是这句话最实在的落点：`sidebarRight` 的打开在同一 tick 读不回来，
		 * 所以它**返回 true 之后**才由有界读回去写状态（见 drawer 分支与
		 * `scheduleRightbarOpenConfirm`）。用户那时可能已经切走 —— 迟到的结论不会再写。
		 */
		function switchLayout(place, appId, env) {
			...
			// 一次**被接受**的切换：动作序号 +1，并把上一条还在等读回的并列打开撤掉 ——
			// 用户已经另作选择了，那条迟到的结论不再有资格写状态
			// （判据见 `drawerIntentStillCurrent`）。
			layoutActionSeq += 1;
			var actionSeq = layoutActionSeq;
			cancelPendingRightbarOpen();
			...
			if (target.key === "panel") {
				...
				recordLayoutPlace(appId, target.key);
				return true;
			}
			if (target.key === "drawer") {
				// 并列那一面走**右栏**：三条通道都认（`betterSidebar` → `sidebarRight` →
				// `details`，优先级与理由写在 `openRightColumn`）。先让它开出来、
				// **读回确认**之后再写状态 —— "界面说切了、其实没切"是这一路反复出现的
				// 失败形态（幽灵座位、`float()` 静默 return 都是它）。
				//
				// **2026-09 的修正**：这里原来只认 `sidebarRight`，于是 0.1.2-rc.1 上
				// （那个版本没有这个服务、却有 `details` + `layout.openDetails`）右侧栏
				// 被判成"打不开"。实测：Inspect 的 client Service 目录里没有 sidebarRight，
				// 但槽位树里 `details` 存在且被 ui-chat 的 DetailsPanel 占着。
				//
				// **2026-10 的修正（真机时序）**：`sidebarRight` 那条路**同一 tick 读不到
				// 不等于没开成** —— 根因与窗口参数写在那组常量上面。所以这里分三步：
				//   ① 有通道、同步读回即确认 ⇒ 老样子，写状态 + true（快路径，没有延迟）；
				//   ② 有通道、`openTab` 发出去了但同步没读到 ⇒ **返回 true**（按这个函数
				//      的返回值语义：动作发出去了），状态**推迟到有界读回确认**才写；
				//   ③ 其余（一条通道都没有 / `openTab` 抛了 / 第三方拒了）⇒ 老样子：如实
				//      false + 按事实分流的那句文案。
				// ②里"窗口用尽"与"用户已经切走"都**绝不写"已并列"**，前者还要如实弹一句。
				var opened = openRightColumn(ctx);
				if (opened.ok) {
					ui.set({ drawer: true, drawerId: appId });
					recordLayoutPlace(appId, target.key);
					return true;
				}
				if (opened.channel === "sidebarRight" && opened.dispatched === true) {
					var modeAtDispatch = ui.get().mode;
					var scheduled = scheduleRightbarOpenConfirm(rightbarService(ctx), function (observed) {
						// 迟到的结论先过竞态：用户可能已经切到别的地方/别的小程序了。
						if (!drawerIntentStillCurrent(actionSeq, appId, modeAtDispatch)) return;
						if (observed.docked === true) {
							// 读到了我们那一格：这时**才**写"已并列"。
							ui.set({ drawer: true, drawerId: appId });
							recordLayoutPlace(appId, target.key);
							return;
						}
						// 窗口用尽还没读到：**如实失败**，并且绝不写"已并列"的假状态。
						if (t !== null) ui.set({ toast: t("open.rightbarOpenFailed") });
					});
					if (scheduled) return true;
					// 连一次重读都排不上（既没有 setTimeout 也没有 Promise）：退回旧的严格
					// 语义 —— 同步读回是唯一判据，如实失败。真机（浏览器）上走不到这里。
					if (t !== null) ui.set({ toast: t("open.rightbarOpenFailed") });
					return false;
				}
				// 走到这里只剩**如实失败**那一种：`channel === "none"`、`openTab` 抛了、
				// 或第三方侧栏拒了这次打开。文案按**事实**分流 —— `none` = 一条通道都没有
				// （换 DSH 版本才有救），有通道却没开成 = 另一件事（用户关掉了这一格 /
				// 第三方拒了），用「没有原生右栏服务」那句话会把人引到错误的线索上。
				if (t !== null) ui.set({ toast: t(opened.channel === "none" ? "open.rightbarUnavailable" : "open.rightbarOpenFailed") });
				return false;
			}
			if (target.key === "corner") {
				...
				//
				// **2026-09 的第二次修正**：原来这里只认 `sidebarRight.float()`，而
				// 0.1.2-rc.1 的 `details` 通道**只有开/关两态、没有浮起** —— 于是悬浮被
				// 判成"打不开"、只弹一条 toast。现在按**能力分派**：
				//
				//   * 有 `float()`（新 DSH）→ 用 DSH 原生浮起，零自绘；
				//   * 只有 `details`（这一版）→ **我们自己画窗口外壳**，内容复用同一份
				//     `MiniAppRightbarPane`。这不是"两套并存"：内容只有一份，外壳有两种。
				//
				// 两条都做不到才算如实失败。
				var capability = rightColumnCapability(ctx);
				if (capability.nativeFloat) {
				...
				// 没有原生浮起 ⇒ **自绘浮窗外壳**。这条路**不依赖任何右栏通道**：
				// 窗口画在 `shell.overlay` 座位里（`MiniAppOverlaySeat`），而
				// `MiniAppFloatWindow` **自带几何**（夹取、拖拽、缩放手柄），
				// 所以 `sidebarRight` / `details` 都不在的构建上它照样能画。
				//
				// **2026-09 的第三次修正**：这里原来要求 `capability.column`，于是
				// "两个右栏通道都没有"被误判成"悬浮打不开" —— 而实际上缺的只是
				// **停靠那一面**（并列），悬浮本来就是自绘的。缺服务时如实失败是对的，
				// 但要失败的只有"我们真做不到的那一面"，不是把能做到的一起放弃。
				ui.set({ corner: true, cornerId: appId });
				recordLayoutPlace(appId, target.key);
				return true;
			}
			// 浏览器新页签在 DSH 之外：**不动任何一个浮层**。用户回来时原来那个面还在原处
			// —— 这是"新开一个"，不是"搬家"。它也不进 `last_place_by_app`：那是
			// "呈现面"的记录，新页签不是（见 LAST_PLACE_KEYS 的说明）。
			...
			recordLayoutPlace(appId, "session");
			return true;
		}

		/**
		 * switchLayout 成功之后顺手把"这次停在了哪个呈现面"记进 prefs。
		 *
		 * 三个要点，都是任务裁定的：
		 *  * **只在成功路径上记**（各分支里 `return true` 之前调）：没切过去就记，
		 *    下次打开会去一个屏幕上根本没出现过的面；
		 *  * **fire-and-forget**（`void`）：写入失败不阻塞、不打断切换 —— 它影响的
		 *    只是"下次打开默认去哪儿"，与 `prefs.pin` 同一条乐观哲学（那边失败了要
		 *    回滚，是图钉的显示必须跟盘上一致；这里 recordPlace 内部自己处理回滚）；
		 *  * browser 不记（`recordPlace` 的白名单会直接忽略它）。
		 */
		function recordLayoutPlace(appId, place) {
			void prefs.recordPlace(appId, place);
		}

		// ------------------------------------------------------ 入口决定落点
		//
		// 「同一个入口打开同一个小程序，默认落在哪儿」的判决（需求②，已拍板）：
		//
		//   1. `last_place_by_app[appId]` 存在 → 用它（尊重用户上一次自己选的形态）；
		//   2. 否则用**来源默认**：
		//      · `session-title`（会话标题栏）→ 会话内。会话内有两个候选（右侧栏
		//        drawer / 会话页签 session），**首次默认 drawer**：drawer 是 DSH 原生
		//        右栏那一格，不替换会话主体（session 页签会把「对话」视图整个换走，
		//        对"顺手打开个小工具"来说太重）；拿不到原生右栏时它已有自己的降级
		//        话术（一条 toast），不需要新逻辑。
		//      · `panel`（全屏浮层，以及其它一切入口）→ panel，维持现状。
		//   3. 来源信息**不持久化**，只活在这一次打开动作里；判决落在 `switchLayout`
		//      的成功路径上之后，`last_place_by_app` 才会更新（于是第 1 条从下次起生效）。
		//
		// 于是首次从标题栏打开 → drawer，并且这次切换本身会把 drawer 写进 last；
		// 用户之后无论在哪个面里手动切到别处 → last 跟着变 → 下次打开尊重新选择。

		/** 打开动作的来源。只认这两个值，别的（含拼写错误）一律按 `panel` 处理。 */
		var OPEN_ORIGIN_SESSION_TITLE = "session-title";
		var OPEN_ORIGIN_PANEL = "panel";

		/**
		 * 「按来源与历史，选一个地方打开」—— `switchLayout(place, …)` 的判决包装。
		 *
		 * 不改 `switchLayout` 本身的签名与语义（五个直接调用点都在用"用户显式选了
		 * 地方"那条路），这里多做的只有**选地方**这一步。
		 *
		 * prefs 还没读到（用户刚进 DSH 就点）时先补读一次再判决：盘上的
		 * `last_place_by_app` 只在 GET 里，不读就判等于把"尊重上次选择"降级成
		 * "永远用来源默认"。补读失败（离线 / 老宿主）也照样判 —— 那时 last 为空，
		 * 自然落回来源默认。已读到时**同步**判决：正常路径上的点击不需要等任何
		 * 往返（那颗图标必须跟手）。
		 *
		 * @returns `switchLayout` 的返回值（动作发出去了吗）。
		 */
		function openFrom(origin, appId, env) {
			var decide = function () {
				var state = prefs.get();
				var last = null;
				var known = state !== null && typeof state === "object" ? state.lastPlaceByApp : null;
				if (known !== null && typeof known === "object") {
					var value = known[appId];
					if (typeof value === "string" && LAST_PLACE_KEYS.indexOf(value) >= 0) last = value;
				}
				var place = last !== null ? last
					: origin === OPEN_ORIGIN_SESSION_TITLE ? "drawer"
					: "panel";
				return switchLayout(place, appId, env);
			};
			if (prefs.get().loaded === true) return decide();
			return prefs.load().then(decide, decide);
		}
		...
		function CardAction(props) {
			...
			// `tone`：静止态的图标颜色（不传就还是次要色，既有调用一个不动）。
			// 它是「固定状态」的**颜色维度**：面板里那枚 📌 已固定时传品牌色，与描边/
			// 实心那个**形状维度**互相独立 —— 色弱或高对比模式下形状仍在，反之亦然。
			var resting = props.tone === undefined ? T.text2 : props.tone;
			return React.createElement("div", Object.assign({
				...
				style: {
					...
					color: danger && hover ? T.danger : hover ? T.text1 : resting,
		...
		/**
		 * 我们那一格的**页签标题**（`sidebar.right.pane.tab.title`）。
		 *
		 * 官方配方里标题与正文是**两次登记、两把同名 key**：不登记标题，页签显示的是
		 * 登记类型时抓下来的 `title(address)` 快照；登记了，语言切换才会跟着走
		 * （`locale: NS` 声明的登记会被运行时装上 `t`，见 ui-renderer 的 `kit['t']`）。
		 */
		function MiniAppRightbarTitle(props) {
			return React.createElement("span", { "data-dsh-miniapp-rightbar-title": "" }, props.t("view.tab"));
		}
		...
		/**
		 * `details` 槽位的那一列 —— **内容与右栏 tab 完全同一份**（`MiniAppRightbarPane`）。
		 *
		 * 这一个组件存在的理由只有一句：把"内容"和"外壳"分开。内容（跑哪个小程序、工具条、
		 * 切换布局）只有一份实现；外壳有两种 —— DSH 的右栏 tab（新版本）与 `details` 列
		 * （这一版）。**这两种外壳不会同时出现**：注册时按能力二选一（见 apply 里那两个
		 * `ctx.effect`）。
		 */
		function MiniAppDetailsColumn(props) {
			var t = props.t;
			return React.createElement("div", {
				"data-dsh-miniapp-details": "",
				style: {
					display: "flex", flexDirection: "column", flex: "1 1 auto",
					minHeight: 0, width: "100%", height: "100%",
					boxSizing: "border-box", background: T.bgLayer1
				}
			},
				React.createElement("div", {
					style: {
					...
					React.createElement("span", {
						style: { fontSize: 13, fontWeight: 600, color: T.text1, flex: 1, minWidth: 0 }
					}, t("bar.title")),
					React.createElement(CardAction, {
						icon: "close", label: t("actions.close"),
						onRun: function () { closeRightColumn(props.ctx); }
					})
				),
				React.createElement(MiniAppRightbarPane, props)
			);
		}

		// --------------------------------------------------------------- 自绘浮窗外壳
		//
		// 这一版 DSH 的 `details` 只有开/关两态、**没有 `float()`**，所以「悬浮」这一面
		// 由我们自己画外壳。**只画外壳**：标题栏、拖动、缩放、夹取、层级；里面的内容
		// 仍是 `MiniAppRightbarPane` —— 与右栏、与 `details` 列共用同一份。
		//
		// 这与 t38 拆掉的"两套并存"不是同一件事：那时候是**两套内容**（自绘 647 行的
		// 运行器 vs DSH 原生），拆得对；现在是**一套内容、两种外壳**，而外壳本身有
		// 真实差异（有 `float()` 就用 DSH 的，没有才自己画）。
		var FLOAT_DEFAULT = { width: 420, height: 560 };
		var FLOAT_MIN = { width: 280, height: 200 };
		var FLOAT_MARGIN = 8;
		/** 初始位置：右上角往内缩一点。与"参考浏览器扩展浮窗"的直觉一致。 */
		function floatDefaultRect() {
			var viewport = viewportSize();
			var width = FLOAT_DEFAULT.width;
			var height = FLOAT_DEFAULT.height;
			if (viewport.width !== undefined && width > viewport.width - FLOAT_MARGIN * 2) width = Math.max(FLOAT_MIN.width, viewport.width - FLOAT_MARGIN * 2);
			if (viewport.height !== undefined && height > viewport.height - FLOAT_MARGIN * 2) height = Math.max(FLOAT_MIN.height, viewport.height - FLOAT_MARGIN * 2);
			var x = viewport.width === undefined ? 24 : viewport.width - width - 24;
			return { x: x < FLOAT_MARGIN ? FLOAT_MARGIN : x, y: 64, width: width, height: height };
		}

		/** 把矩形夹回视口内（拖动/缩放都可能把它推出界）。读不到视口就不夹，不猜。 */
		function clampFloatRect(rect) {
			var viewport = viewportSize();
			var width = Math.max(FLOAT_MIN.width, rect.width);
			var height = Math.max(FLOAT_MIN.height, rect.height);
			if (viewport.width !== undefined) width = Math.min(width, Math.max(FLOAT_MIN.width, viewport.width - FLOAT_MARGIN * 2));
			if (viewport.height !== undefined) height = Math.min(height, Math.max(FLOAT_MIN.height, viewport.height - FLOAT_MARGIN * 2));
			var x = Number.isFinite(rect.x) ? rect.x : 0;
			var y = Number.isFinite(rect.y) ? rect.y : 0;
			if (viewport.width !== undefined) x = Math.min(Math.max(x, FLOAT_MARGIN), Math.max(FLOAT_MARGIN, viewport.width - width - FLOAT_MARGIN));
			if (viewport.height !== undefined) y = Math.min(Math.max(y, FLOAT_MARGIN), Math.max(FLOAT_MARGIN, viewport.height - height - FLOAT_MARGIN));
		 ...
		 * 自绘悬浮窗外壳。
		 *
		 * 拖动与缩放**都走 pointer 事件 + `setPointerCapture`**，而不是 `mousemove`：
		 * 这一版在真机上被 pointer capture 咬过一次 —— 详见 `#onGripPointerDown` 的注释。
		 */
		function MiniAppFloatWindow(props) {
			var t = props.t;
			var [rect, setRect] = useState(function () { return clampFloatRect(props.initialRect === undefined ? floatDefaultRect() : props.initialRect); });
			var dragRef = useRef(null);

			// 视口变化（窗口缩放）时把它夹回来，别让它跑到看不见的地方。
			useEffect(function () {
				if (typeof window === "undefined" || typeof window.addEventListener !== "function") return undefined;
				function onResize() { setRect(function (current) { return clampFloatRect(current); }); }
				window.addEventListener("resize", onResize);
				return function () { window.removeEventListener("resize", onResize); };
			}, []);

			/**
			 * 开始拖动。`event.preventDefault()` 在这里**是安全的**：标题栏不是按钮 ——
			 * 关闭按钮在标题栏**里面**，所以必须先判"点的是不是交互元素"，
			 * 否则按在关闭按钮上的那一下会被这里吞掉（真机实测：按钮完全点不动）。
			 */
			function onHeaderPointerDown(event) {
				if (event.button !== undefined && event.button !== 0) return;
				var target = event.target;
				if (target !== null && target !== undefined && typeof target.closest === "function") {
					if (target.closest("button, a, input, select, textarea, [role=\"button\"], [role=\"link\"]") !== null) return;
				}
			...
			/**
			 * 两种手势共用一套：记下起点 → 指针移动时改矩形 → 抬手结束。
			 *
			 * move/up 的监听写在下面那个 `useEffect` 里、挂 **window**；这里只负责记起点。
			 */
			...
			/**
			 * 指针移动 → 改矩形。**挂在 window 上，不挂元素上。**
			 *
			 * 真机上踩过一次：原来把 `onPointerMove` 挂在标题栏元素上、并试图
			 * `setPointerCapture`。CDP 的合成鼠标事件下 pointer capture 拿不到，
			 * move 事件于是**根本到不了那个元素** —— 表现就是"按得下去、窗口纹丝不动"，
			 * 而 `defaultPrevented` 还是 true（处理函数确实跑了），所以从外表看毫无线索。
			 * 挂在 window 上就没有这个前提：只要指针动了就收得到，不需要捕获成功。
			 */
			function onWindowPointerMove(event) {
				...
				var next;
				if (drag.kind === "resize") {
					next = { x: drag.start.x, y: drag.start.y, width: drag.start.width + dx, height: drag.start.height + dy };
				} else {
					next = { x: drag.start.x + dx, y: drag.start.y + dy, width: drag.start.width, height: drag.start.height };
				}
				setRect(clampFloatRect(next));
			}

			function endDrag() { dragRef.current = null; }

			// 拖动期间才挂 window 监听；抬手/取消/卸载都摘掉 —— 不许留下常驻监听。
			useEffect(function () {
				if (typeof window === "undefined" || typeof window.addEventListener !== "function") return undefined;
			...
			return React.createElement("div", {
				...
				style: {
					...
					borderRadius: 12, border: "1px solid " + T.border1, background: T.bgLayer1,
					boxShadow: "0 12px 32px rgba(0,0,0,.28)", boxSizing: "border-box"
				}
			},
				React.createElement("div", {
					...
					style: {
						display: "flex", alignItems: "center", gap: 8, flex: "0 0 auto",
						padding: "8px 10px", cursor: "move", touchAction: "none",
						borderBottom: "1px solid " + T.border1, background: T.bgLayer2
					}
				},
					React.createElement("span", {
						style: { fontSize: 12, fontWeight: 600, color: T.text1, flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }
					}, t("bar.title")),
					React.createElement(CardAction, {
						icon: "close", label: t("actions.close"),
						// 关掉悬浮 = 收回 `corner`，回落到**全屏面板**（不是 hidden）：
						// 关掉一个面不该让小程序整个消失。
						onRun: function () { props.ui.set({ open: true }); }
					})
				),
				React.createElement("div", {
					...
					React.createElement(MiniAppRightbarPane, props),
					React.createElement("div", grip, React.createElement("span", { style: { color: T.text2, display: "flex", pointerEvents: "none" } },
						React.createElement(Icon, { name: "corner", size: 12 })))
				)
			);
		}
		...
		function MiniAppOverlaySeat(props) {
			...
			useEffect(function () {
				// **必须走双通道**（`openRightColumn` / `closeRightColumn`），不能再用只认
				// `sidebarRight` 的 `openRightbarPane` / `closeRightbarPane`。
				//
				// 真机上踩过这一次：`switchLayout` 已经改用 `openRightColumn` 把 `details`
				// 列打开了，而这条 effect 仍在调单通道版 —— 单通道版在 details 通道上恒失败、
				// `closeRightbarPane` 恒 `no-service`，于是它**把刚打开的列立刻又关掉**。
				// 表现是"点了在右侧打开、collapsed 始终 true"，而代码看起来两处都没错。
				// 教训：同一件事有两个入口时，改一处必须搜全部调用点，否则留下一条互相打架的路。
				//
				// **打开还在等读回时不许关**（2026-10）：那条有界读回要读的正是"刚发出去的
				// 那一格"，在这里 close 一下就把自己等的对象撤掉了（见
				// `scheduleRightbarOpenConfirm` / `rightbarOpenPending`）。正常路径上本条
				// effect 在窗口内**不会**重跑（窗口里不写 ui，依赖表不动）；这一句是给
				// "因为别的原因重渲染了一次"那一类情况兜底 —— 那时也不许把这次打开撤掉。
				if (state.drawer === true && state.drawerId !== null) openRightColumn(props.ctx);
				else if (!rightbarOpenPending()) closeRightColumn(props.ctx);
				// 卸载时把那一列收回去（用户切走时不该留一个空壳占着右栏）。
				return function () { if (!rightbarOpenPending()) closeRightColumn(props.ctx); };
			}, [state.drawer, state.drawerId, props.ctx]);

			// 悬浮那一面**按能力分派**（2026-09 第二次修正）：
			//   * 有 `sidebarRight.float()`（新 DSH）→ 由 DSH 画，我们只做读回确认
			//     （见 switchLayout 的 corner 分支），这里**不画**，否则会画出第二个窗口；
			//   * 只有 `details` 通道（0.1.2-rc.1）→ **我们自己画外壳**（`MiniAppFloatWindow`），
			//     内容仍是同一份 `MiniAppRightbarPane`。
			//
			// 判据必须**读能力**而不是读 `state.corner`：状态说"该浮着"、
			// 而 DSH 正在画，这两件事在这条分支上恰好同时成立，用状态区分必然画重。
			var capability = props.ctx === undefined || props.ctx === null
				? { nativeFloat: false, column: false }
				: rightColumnCapability(props.ctx);
			// **只排除"DSH 正在替我们画"那一种**（`nativeFloat`），不再要求 `capability.column`。
			// 自绘浮窗不需要任何右栏通道（`MiniAppFloatWindow` 自带几何）；把它门控在
			// `capability.column` 上，正是"悬浮被判成打不开、连自绘都不画"的根因。
			var floating = state.corner === true && state.cornerId !== null && !capability.nativeFloat;
			// 自绘浮窗与全屏浮层**互斥**（`nextPresentation` 保证只有一个面亮着），
			// 但这里仍各判各的：状态万一被写歪，画两个叠层比少画一个更难查。
			var surface = floating
				? React.createElement(MiniAppFloatWindow, Object.assign({}, props, {
					ui: props.ui,
					initialRect: undefined
				}))
				: React.createElement(MiniAppOverlay, props);
		...
		/**
		 * 胶囊主段最多直接显示几枚固定图标；再多就收敛成「前 N 枚 + +k」。
		 *
		 * 为什么是 4 而不是"全显示 8 个"或横向滚动：
		 *  * 标题栏是会话头部里最挤的一行（会话标题、日志按钮都在抢宽度），一颗 4 枚
		 *    图标的胶囊 ≈ 116px，与"两颗 32px 按钮"同量级，再宽就开始吃会话标题了；
		 *  * 标题栏**不是能滚的地方**（鼠标不会停在那儿等横向滚动），放不下就收敛，
		 *    「+k」既保住了"还有几个"的信息，又给了一个打开面板看全部的入口；
		 *  * 上限 8 与显示 4 是两个独立的数：上限管"固定多少合理"，显示管"标题栏
		 *    放得下多少"，恰好相等只是巧合，不该用同一个常量把它们焊死。
		 */
		var BAR_PIN_VISIBLE_MAX = 4;
		/**
		 * 胶囊两段的几何：整颗高 32（与标题栏其它按钮同档），主段图标格 26 宽、
		 * 图标 14px，下拉段 28 宽、▾ 14px；中间一道 1px 竖缝把它们明显分开
		 * （「一颗胶囊分两段」的分段感就靠它，没有缝两段就是一整块）。
		 */
		var BAR_PILL_HEIGHT = 32;
		var BAR_PILL_ICON_WIDTH = 26;
		var BAR_PILL_CHEVRON_WIDTH = 28;
		 ...
		 */
		// 小程序栏 ⋮ 菜单里的落点。**悬浮（corner）原来漏在这里** —— 那是
		// 「悬浮面板打不开」的直接原因之一：五处布局里另外四处都能从某个入口切过去，
		// 唯独 `corner` 在标题栏这条路上根本没有按钮，用户看不到它、自然也切不过去。
		// 补上之后它与 `LAYOUT_PLACES` 的五个地方一一可达。
		var BAR_MENU_ITEMS = [
			{ place: "drawer", icon: "side", labelKey: "bar.open.drawer" },
			{ place: "corner", icon: "corner", labelKey: "bar.open.corner" },
		...
		/**
		 * 小程序栏两侧各自要画什么：标题栏胶囊主段要的**固定记录列表**（按固定的
		 * 先后排序，目录里找不到的跳过），以及「全部小程序」那一栏的列表。
		 *
		 * 「全部」**就是全部**（含已经固定的那些）：它顶着「全部小程序」这个标题，
		 * 把它理解成"除固定之外的"就等于让标题说假话。
		 *
		 * 固定列表里**找不到的那一条直接不显示**（而不是抛、更不是把它从盘上抹掉）：
		 * **用户可能只是暂时把它删了**，界面上当作"没有这个固定"，但绝不去改磁盘上
		 * 那个值（见 MiniAppBar 里的说明）。
		 *
		 * `pinnedIds` 原样交回（含目录里找不到的）：面板每一行的 📌 两态按它判，
		 * "目录里暂时没有"不该让那一行看起来"没固定过"。
		 */
		function splitBarApps(apps, pinnedAppIds) {
			var list = Array.isArray(apps) ? apps : [];
			var ids = Array.isArray(pinnedAppIds) ? pinnedAppIds : [];
			var pinnedApps = [];
			for (var index = 0; index < ids.length; index += 1) {
				var wanted = ids[index];
				if (typeof wanted !== "string" || wanted === "") continue;
				var found = null;
				for (var inner = 0; inner < list.length; inner += 1) {
					var app = list[inner];
					if (app !== null && typeof app === "object" && app.miniapp_id === wanted) {
						found = app;
						break;
					}
				}
				if (found !== null) pinnedApps.push(found);
			}
			return { pinnedApps: pinnedApps, pinnedIds: ids, all: list };
		}
		...
		function MiniAppBarRow(props) {
			...
			var row = React.createElement("div", {
				...
				React.createElement("div", {
					...
					React.createElement(CardAction, {
						// 固定状态的**两个维度**，必须同时成立（只靠任何一个都不够）：
						//  * 形状：实心 `pin` = 已固定 / 描边 `pinOutline` = 还没固定
						//    （同一个几何的两态，不是"变淡" —— 降透明度是禁用，不是未选中）；
						//  * 颜色：已固定 = 品牌色 `T.brand` / 未固定 = 次要色 `T.text2`。
						//    色弱与高对比模式下形状那一维仍然可辨；主题把品牌色改得与次要色
						//    接近时形状那一维顶上。`aria-pressed` 是读屏与测试用的第三维。
						icon: pinned ? "pin" : "pinOutline",
						stroke: pinned !== true,
						tone: pinned ? T.brand : undefined,
		 ...
		 * 下拉面板。
		 *
		 * 自上而下：标题行（小程序 + ✕）、全部小程序、分隔线、管理小程序。
		 *
		 * **不再有「固定的小程序」分区**：固定的那几个已经直接显示在标题栏胶囊的
		 * 主段里（可点、可辨认固定态），面板里再开一个固定区只会把同一条小程序画
		 * 两遍 —— 固定与否在「全部」每一行的那枚 📌 上就能看出来。
		 *
		 * 身体状态各有各的话，**都不留天窗**：
		 *  * 目录读失败 → 一条错误（它比"空"更该说清楚，否则用户以为自己的小程序没了）；
		 *  * 还在读 → `list.loading`；
		 *  * 真的一条都没有 → `bar.empty`（并指向最下面那个入口）；
		 *  * 有内容 → 「全部小程序」一个分区（含已固定的那些，标题不说假话）。
		 *
		 * 另有一条与目录状态无关的提示行：`prefs.notice === "pin-limit"` 时画出
		 * 「最多固定 8 个」（点 📌 撞上限不是静默失败，要说出来）。
		 */
		function MiniAppBarPanel(props) {
			...
			var sections = splitBarApps(catalog.apps, prefsState.pinnedAppIds);

			// 点面板外面 / 按 Esc 都关掉。"里面"算整条小程序栏（见 outsideEvent）。
			useDismiss(true, "[data-dsh-miniapp-bar]", props.onClose);

			// 行点击 = 打开它。落点走 `openFrom` 的判决（标题栏这个入口的来源是
			// `session-title`）：这个 app 上次被摆进哪个呈现面就去哪儿，没有记录时
			// 首次默认右侧栏（见 openFrom 旁的说明）。
			var openApp = function (appId) {
				props.onClose();
				void openFrom(OPEN_ORIGIN_SESSION_TITLE, appId, { t: t, ctx: props.ctx });
			};
			// ⋮ 菜单是用户**显式**选了地方，不走判决，直接切。
			...
			var pin = function (appId) {
				...
				// 撞上限时 `prefs.pin` 自己会留 notice，面板把它画出来。
			...
			var row = function (app, key) {
				var pinned = sections.pinnedIds.indexOf(app.miniapp_id) >= 0;
				return React.createElement(MiniAppBarRow, {
					key: key, t: t, app: app, pinned: pinned,
					onOpen: openApp, onPin: pin, onSwitch: runPlace
				});
			};

			var body = [];
			if (prefsState.notice === "pin-limit") {
				body.push(note(t("bar.pinLimit", { max: PINNED_MAX }), T.warn));
			}
			if (catalog.error !== null) {
				...
				for (var index = 0; index < sections.all.length; index += 1) {
					body.push(row(sections.all[index], "all-" + index));
				}
			}
		...
		/**
		 * 胶囊主段里的一枚固定图标：它就是"固定的那一个小程序"在标题栏上的化身。
		 *
		 * 固定状态的**两个维度**（与面板行里那枚 📌 同一套视觉语言，理由见
		 * `MiniAppBarRow` 里 📌 那段注释）：
		 *  * 形状：图标底下有一条**品牌色短横线**（下划线）。emoji 自带颜色、描边
		 *    那套对它无意义，所以标题栏这一侧的形状维度落在下划线的有无上；
		 *  * 颜色：`T.brand`（固定）—— 0 个固定时的通用入口是 `T.text2` 且无下划线，
		 *    两者的差别不依赖颜色深浅这一个维度。
		 * 点击它 = 打开**这一个**小程序（`openFrom`，来源 `session-title`）。
		 */
		function MiniAppPinnedIcon(props) {
			var [hover, setHover] = useState(false);
			var app = props.app;
			var icon = String(app.icon === null || app.icon === undefined ? "" : app.icon).trim();
			return React.createElement("div", {
				role: "button", tabIndex: 0,
				title: props.label, "aria-label": props.label,
				"data-dsh-miniapp-bar-part": "pin-open",
				"data-dsh-miniapp-bar-app": app.miniapp_id,
				onClick: props.onRun,
				onKeyDown: function (event) {
					if (event.key === "Enter" || event.key === " ") { event.preventDefault(); props.onRun(); }
				},
				...
				style: {
					...
					gap: 1, width: BAR_PILL_ICON_WIDTH, height: "100%",
					flex: "0 0 auto", cursor: "pointer",
					color: hover ? T.text1 : T.brand,
					background: hover ? T.hoverSurface : "transparent",
					transition: "color 120ms, background 120ms"
				}
			},
				React.createElement("span", {
					"aria-hidden": "true",
					style: {
						display: "grid", placeItems: "center", width: 20, height: 16,
						fontSize: 14, lineHeight: 1, color: T.brand
					}
				}, icon.length > 0 ? icon : React.createElement(Icon, { name: "app", size: 12 })),
				// 固定态的形状维度：品牌色短横线（未固定的通用入口没有这条线）。
				React.createElement("span", {
					"aria-hidden": "true",
					style: { width: 12, height: 2, borderRadius: 1, background: T.brand }
				})
			);
		}
		 ...
		 * 胶囊主段放不下时的溢出标记「+k」。
		 *
		 * 它不是某一枚固定图标（点它不打开任何一个小程序），点它**开面板** ——
		 * 面板里的「全部小程序」就是"剩下那几个"能被看见与点开的地方。标题栏
		 * 不做横向滚动：那里不是鼠标会停下来等滚动的地方。
		 */
		function MiniAppPinOverflowTag(props) {
			var [hover, setHover] = useState(false);
			return React.createElement("div", {
				role: "button", tabIndex: 0,
				title: props.label, "aria-label": props.label,
				"data-dsh-miniapp-bar-part": "pin-overflow",
				onClick: props.onRun,
				onKeyDown: function (event) {
					if (event.key === "Enter" || event.key === " ") { event.preventDefault(); props.onRun(); }
				},
				...
				style: {
					...
					flex: "0 0 auto", padding: "0 6px", cursor: "pointer",
					font: "inherit", fontSize: 11, fontWeight: 600, lineHeight: 1,
					color: hover ? T.text1 : T.text2,
					background: hover ? T.hoverSurface : "transparent",
					transition: "color 120ms, background 120ms"
				}
		 ...
		 * 0 个固定时主段的那枚通用入口。
		 *
		 * 与 `MiniAppPinnedIcon` 的差别就是**未固定**的两个维度：次要色 + 没有下划线。
		 * 点它开面板（此刻主段里没有"某一个"可打开，面板才是入口）。
		 */
		function MiniAppPinNoneButton(props) {
			var [hover, setHover] = useState(false);
			return React.createElement("div", {
				role: "button", tabIndex: 0,
				title: props.label, "aria-label": props.label,
				"data-dsh-miniapp-bar-part": "pin-none",
				onClick: props.onRun,
				onKeyDown: function (event) {
					if (event.key === "Enter" || event.key === " ") { event.preventDefault(); props.onRun(); }
				},
				...
				onMouseLeave: function () { setHover(false); },
				style: {
					display: "grid", placeItems: "center", width: BAR_PILL_ICON_WIDTH, height: "100%",
					flex: "0 0 auto", cursor: "pointer",
					color: hover ? T.text1 : T.text2,
					background: hover ? T.hoverSurface : "transparent",
					transition: "color 120ms, background 120ms"
		 ...
		 * 胶囊的下拉段（▾）：开关面板的那一半。
		 *
		 * `aria-expanded` / `aria-haspopup` 挂在它身上（不是主段）：主段的每一枚
		 * 图标各有自己的可访问名（"打开谁"），而"这里还有一层可展开"是下拉段单独
		 * 的语义。弹层本身的 role 是 dialog，所以 haspopup 照 ARIA 1.2 给同一个值。
		 */
		function MiniAppPillChevron(props) {
			var [hover, setHover] = useState(false);
			return React.createElement("div", {
				role: "button", tabIndex: 0,
				title: props.label, "aria-label": props.label,
				...
				onMouseLeave: function () { setHover(false); },
				style: {
					display: "grid", placeItems: "center", width: BAR_PILL_CHEVRON_WIDTH, height: "100%",
					flex: "0 0 auto", cursor: "pointer",
					color: hover ? T.text1 : T.text2,
					background: hover ? T.hoverSurface : "transparent",
					transition: "color 120ms, background 120ms"
		...
		/**
		 * 标题栏上那颗胶囊：主段（固定的小程序们）+ 明显分隔 + 下拉段（▾）。
		 *
		 * 一颗胶囊分两段，而不是两颗独立的按钮：两段属于**同一个东西**（小程序栏），
		 * 视觉上长在一起才像一个控件 —— 分开摆就会长得像"两个无关的功能"。中间那道
		 * 1px 竖缝负责把"两段"说出来（没有缝就是一整块，看不出哪里该点哪儿）。
		 *
		 * 主段的收敛规则：最多直接显示 `BAR_PIN_VISIBLE_MAX` 枚图标，再多显示
		 * 「+k」（点它开面板）；0 枚时主段是一枚通用入口（开面板），不空着。
		 */
		function MiniAppBarPill(props) {
			var t = props.t;
			var pinnedApps = Array.isArray(props.pinnedApps) ? props.pinnedApps : [];
			var visible = pinnedApps.slice(0, BAR_PIN_VISIBLE_MAX);
			var overflowCount = pinnedApps.length - visible.length;

			// 主段：固定图标按用户固定的先后排（splitBarApps 里已经按 ids 的顺序取好了）。
			var mainChildren = [];
			if (visible.length === 0) {
				mainChildren.push(React.createElement(MiniAppPinNoneButton, {
					key: "none", label: t("bar.entry"), onRun: props.onToggle
				}));
			} else {
				for (var index = 0; index < visible.length; index += 1) {
					var app = visible[index];
					mainChildren.push(React.createElement(MiniAppPinnedIcon, {
						key: app.miniapp_id,
						app: app,
						label: t("bar.openPinned", { name: app.name }),
						// 闭包而不是当场调用：这里要交出去的是"点下去做什么"，
						// 不是那件事做完的结果（当场调用只会把 undefined 交出去）。
						onRun: function () { props.onOpenApp(app.miniapp_id); }
					}));
				}
				if (overflowCount > 0) {
					mainChildren.push(React.createElement(MiniAppPinOverflowTag, {
						key: "overflow",
						count: overflowCount,
						label: t("bar.morePinned", { count: overflowCount }),
						onRun: props.onToggle
					}));
				}
			}

			return React.createElement("div", {
				"data-dsh-miniapp-bar-part": "pill",
				role: "group",
				"aria-label": t("bar.entry"),
				style: {
					display: "inline-flex", alignItems: "stretch",
					height: BAR_PILL_HEIGHT, flex: "0 0 auto",
				...
				// 主段。
				React.createElement("div", {
					style: { display: "flex", alignItems: "center", flex: "0 0 auto", padding: "0 2px" }
				}, mainChildren),
				// 中间那道明显的分隔：胶囊分两段的"分"就靠它。
				React.createElement("span", {
					"aria-hidden": "true",
					style: { flex: "0 0 1px", width: 1, background: T.border2, margin: "6px 0" }
				}),
				// 下拉段。
				React.createElement(MiniAppPillChevron, {
					label: t("bar.entry"), open: props.open, onRun: props.onToggle
				})
			);
		}

		/**
		 * 标题栏上的小程序入口：一颗胶囊（主段 = 固定的小程序们，下拉段 = ▾）+ 下拉面板。
		 *
		 * 主段里点**某一枚**图标 = 打开那一个小程序（落点走 `openFrom` 的判决，
		 * 来源 `session-title`：这个 app 上次停在哪个呈现面就去哪儿，没有记录时
		 * 首次默认右侧栏）。点 ▾ = 开面板，面板自上而下是"全部小程序 / 管理小程序"
		 * —— 固定的那几个已经在主段里了，面板里不再有单独的固定分区。
		 *
		 * 固定的某一条已经不在目录里（被删了）：主段跳过它（`splitBarApps` 找不到
		 * 就不返回），但**绝不去改 prefs** —— 用户可能只是暂时删了它，下次再建一个
		 * 同 id 的（或者他改主意了）就该原样回来。
		 */
		function MiniAppBar(props) {
			...
			// 目录没拉过就拉一次（与浮层同一条写法，同一个理由）：主段要显示固定的
			// 那几个的 emoji、面板要列出全部小程序，而用户完全可能一进 DSH 就直接
			// 点这颗胶囊 —— 那一刻目录与 prefs 都可能压根没被拉过。
			useEffect(function () {
				if (catalog.loaded === true || catalog.loading === true) return;
				void appCatalog.load(false);
			}, [catalog.loaded, catalog.loading]);
			...
			var openApp = function (appId) {
				// 主段图标打开小程序时顺手把面板收掉（若开着）：小程序已经打开了，
				// 面板再挂在屏幕上就是挡路的。
				close();
				void openFrom(OPEN_ORIGIN_SESSION_TITLE, appId, { t: t, ctx: ctx });
			};

			return React.createElement("div", {
				// 整条栏（含胶囊与面板）的标记：外面点击那条判决按它认"里面"。
				"data-dsh-miniapp-bar": "",
				ref: barRef,
				// `flex: "0 0 auto"`：头部挤的时候该被压缩的是别的东西，不是这颗胶囊。
				style: { display: "flex", alignItems: "center", gap: 2, flex: "0 0 auto" }
			},
				React.createElement(MiniAppBarPill, {
					t: t,
		...
		function apply(ctx) {
			try {
				...
				// 这一版 DSH 的右栏：`details` 槽位 + `ctx.layout.openDetails()`。
				//
				// **只在没有 `sidebarRight` 时登记** —— 两条通道都登记的话，在新 DSH 上
				// 我们会同时占住两个右栏位（一个原生 tab、一个 details 列），用户会看到
				// 两列东西。这是"能力分派"，不是"两套并存"：任何时刻只有一条路在。
				//
				// 登记 `details` 会**顶掉** ui-chat 的 DetailsPanel（工具详情）—— 这是这一格
				// 的固有代价（kind:single + shadows-shipped-ui），我们在 UI 上如实告知，
				// 卸载时按事实收回（`detailsShadowed` 只在真的登记成功后才为真）。
				ctx.effect(function () {
					if (rightbarService(ctx) !== null) return undefined;
					if (detailsChannel(ctx) === null) return undefined;
					var disposer = ctx.slots.inject(DETAILS_SLOT, function () {
						var registered = ctx.slots.register({
							name: DETAILS_SLOT,
							id: RIGHTBAR_VIEW_ID,
							priority: DETAILS_PRIORITY,
							locale: NS,
							// **不要声明 `children`**。这里踩过一次真机级的事故：
							// 起初照抄了 DSH ui-chat 那格的 `children: { "conversation.details.tool": … }`，
							// 结果是 **整个 DSH 前端起不来** ——
							//   slot "conversation.details.tool" is already declared
							//   (by an entry in "details" (dsh-miniapp))
							// 因为 ui-chat 自己也声明了那个子座位，而我们**遮蔽了它、它照样注册**，
							// 两处声明同一子座位 ⇒ 加载器直接判死，报 "Failed to load plugins"。
							// 子座位的声明归 ui-chat；我们只借这一格来画，不碰它的树。
							inject: function () { return { ctx: ctx, ui: ui }; }
						}, MiniAppDetailsColumn);
						detailsShadowed = true;
						return registered;
					});
					return function () {
						detailsShadowed = false;
						disposer();
					};
				});

				// 右栏（方案 B 的"并列"那一面）：**登记一次、常驻**。
				//
				...
				//
				// 两块**分开**，因为它们的条件不同：
				//   * 正文与标题那两格（keyed 座位）**无条件登记** —— 纯加法，服务不在就没人看得见；
				//   * **类型只登记到一方**（见下面的分派）。两处都登记会在用户眼前变成两个
				//     同名条目：第三方那份由它镜像成原生类型（指南里一行），我们那份再登记一次
				//     又是一行 —— 而「谁能开出来」其实只有一条路。
				ctx.effect(function () {
					var disposers = [];
					// 正文（官方配方：类型进 `sidebarRightTabs`、正文进这一格）。
					...
					// 标题（同一把 key 的第二次登记；页签文字跟着语言表走）。
					//
					// 两样都给，各管一件事，不是重复：`locale: NS` 让这一格在**切语言时重渲染**
					// （ui-renderer 按 locale 版本给出口挂 `useLocaleRevision`），`inject` 交出
					// 它读的那个 `t`（本仓库的规矩：每个登记都写清自己交出去什么）。
					// 两个 `t` 是同一个东西（`locale.bind(NS)` 是活函数、按当前语言查表）。
					disposers.push(ctx.slots.inject(RIGHTBAR_TITLE_SLOT, function () {
						return ctx.slots.register({
							name: RIGHTBAR_TITLE_SLOT,
							id: RIGHTBAR_VIEW_ID,
							key: RIGHTBAR_VIEW_ID,
							locale: NS,
							inject: function () { return { t: t }; }
						}, MiniAppRightbarTitle);
					}));
					// 类型登记：**只登记到一方**。
					//
					// 第三方在时只给它：它会把这一格镜像成原生 tab 类型（kind 与我们的
					// `RIGHTBAR_VIEW_KIND` 逐字相同），所以能力一点没丢；而两处都登记会在
					// `+` 菜单里出现两个同名条目，用户点哪个都能开出同一格 —— 那是噪音，
					// 不是能力。它不在时才登记原生那一份。
					var better = betterSidebarChannel(ctx);
					if (better !== null) {
						try {
							disposers.push(registerBetterSidebarTab({ service: better.service, t: t, ctx: ctx, ui: ui }));
						} catch (error) {
							// 重复 id 会抛（第三方有意如此）；那是"已经登记过了"，不是致命错。
							console.error("[dsh-miniapp] betterSidebar tab register failed:", error);
						}
					} else {
						var tabs = rightbarTabsService(ctx);
						if (tabs !== null && typeof tabs.register === "function") {
							try {
								var disposeType = tabs.register(nativeRightbarDefinition(t));
								// 登记的 disposer 要收好：一个类型登记就是一次注册（注册即 effect），
								// 原来把它丢掉了 —— 插件卸载后类型还留在注册表里（HMR 下尤其明显）。
								if (typeof disposeType === "function") disposers.push(disposeType);
							} catch (error) {
				...
				// 「并列」那条**有界读回**的定时器也要归插件管：插件停用（或 HMR 换一份）
				// 时必须把它清掉，否则一个已经没人要的验证会在后来某个时刻写 ui ——
				// 那正是"幽灵座位"的形状。它按模块级那条记录取，不累积 disposer。
				ctx.effect(function () {
					return function () { cancelPendingRightbarOpen(); };
				}, "dsh-miniapp: pending rightbar open confirm");
			} catch (error) {
		...
		exports.RIGHTBAR_TITLE_SLOT = RIGHTBAR_TITLE_SLOT;
		exports.RIGHTBAR_VIEW_ID = RIGHTBAR_VIEW_ID;
		exports.RIGHTBAR_VIEW_KIND = RIGHTBAR_VIEW_KIND;
		exports.RIGHTBAR_GUIDE_ORDER = RIGHTBAR_GUIDE_ORDER;
		exports.observeRightbar = observeRightbar;
		exports.openRightbarPane = openRightbarPane;
		// ── 第三方侧栏通道（`dsh-better-sidebar`）：探测、登记、打开，全部可达。 ──
		exports.BETTER_SIDEBAR_SERVICE = BETTER_SIDEBAR_SERVICE;
		exports.betterSidebarService = betterSidebarService;
		exports.betterSidebarChannel = betterSidebarChannel;
		exports.registerBetterSidebarTab = registerBetterSidebarTab;
		exports.openBetterSidebarPane = openBetterSidebarPane;
		exports.nativeRightbarDefinition = nativeRightbarDefinition;
		exports.MiniAppRightbarTitle = MiniAppRightbarTitle;
		// ── 双通道右栏（2026-09）：`details` 通道 + 自绘浮窗外壳。导出给测试直接钉。 ──
		exports.layoutService = layoutService;
		exports.detailsChannel = detailsChannel;
		exports.openRightColumn = openRightColumn;
		exports.closeRightColumn = closeRightColumn;
		exports.rightColumnCapability = rightColumnCapability;
		exports.MiniAppDetailsColumn = MiniAppDetailsColumn;
		exports.MiniAppFloatWindow = MiniAppFloatWindow;
		exports.floatDefaultRect = floatDefaultRect;
		exports.clampFloatRect = clampFloatRect;
		exports.DETAILS_SLOT = DETAILS_SLOT;
		exports.DETAILS_PRIORITY = DETAILS_PRIORITY;
		exports.floatRightbarPane = floatRightbarPane;
		// ── 「并列」打开的有界读回（2026-10 真机时序修正）：窗口参数与"还在等吗"
		//    都露出来 —— 有界性（N / 间隔 / 总时长）与"结论只下一次"这两条必须在
		//    没有真机的测试里可断言，否则它们只能活在注释里。 ──
		exports.RIGHTBAR_OPEN_CONFIRM_ATTEMPTS = RIGHTBAR_OPEN_CONFIRM_ATTEMPTS;
		exports.RIGHTBAR_OPEN_CONFIRM_INTERVAL_MS = RIGHTBAR_OPEN_CONFIRM_INTERVAL_MS;
		exports.rightbarOpenPending = rightbarOpenPending;
		...
		exports.BAR_PIN_VISIBLE_MAX = BAR_PIN_VISIBLE_MAX;
		exports.BAR_PILL_HEIGHT = BAR_PILL_HEIGHT;
		...
		exports.MiniAppBarPill = MiniAppBarPill;
		exports.MiniAppPinnedIcon = MiniAppPinnedIcon;
		exports.MiniAppPinOverflowTag = MiniAppPinOverflowTag;
		exports.MiniAppPinNoneButton = MiniAppPinNoneButton;
		exports.MiniAppPillChevron = MiniAppPillChevron;
		exports.MiniAppBarPanel = MiniAppBarPanel;
		exports.MiniAppBarRow = MiniAppBarRow;
		exports.MiniAppBarMenu = MiniAppBarMenu;
		// 固定（走宿主的 /prefs）：多选判决、响应解析、上限、白名单与那一份模块级状态。
		exports.MiniAppPrefs = MiniAppPrefs;
		exports.prefs = prefs;
		exports.togglePinned = togglePinned;
		exports.readPrefsShape = readPrefsShape;
		exports.PINNED_MAX = PINNED_MAX;
		exports.LAST_PLACE_KEYS = LAST_PLACE_KEYS;
		// 入口决定落点：来源常量 + 判决包装（switchLayout 本身保持原签名原语义）。
		exports.OPEN_ORIGIN_SESSION_TITLE = OPEN_ORIGIN_SESSION_TITLE;
		exports.OPEN_ORIGIN_PANEL = OPEN_ORIGIN_PANEL;
