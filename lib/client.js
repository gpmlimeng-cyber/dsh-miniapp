	...
	factory: (require) => {
		...
		function openRightColumn(ctx) {
			...
			if (sidebarRight !== null && typeof sidebarRight.openTab === "function") {
				...
				return { ok: step.ok, channel: "sidebarRight", reason: step.reason, observed: step.observed };
			}
		...
		function switchLayout(place, appId, env) {
			...
			if (target.key === "drawer") {
				...
				if (!opened.ok) {
					// 文案按**事实**分流：`none` = 一条通道都没有（换 DSH 版本才有救），
					// 有通道却没开成 = 另一件事（用户关掉了这一格 / 第三方侧栏拒了这次打开），
					// 用「没有原生右栏服务」那句话会把人引到错误的线索上。
		...
		function MiniAppOverlaySeat(props) {
			...
			useEffect(function () {
				...
				else closeRightColumn(props.ctx);
				// 卸载时把那一列收回去（用户切走时不该留一个空壳占着右栏）。
				return function () { closeRightColumn(props.ctx); };
