...
//
// 于是「同一个小程序」有五个跑的地方：全屏面板、右侧栏、本会话页签、会话右上角浮窗、
// 浏览器新页签。前四个跑的是**同一个运行页组件**（RunnerView）—— 同一条沙箱串、
// 同一个 `src=SERVE/…`，只有露出来的动作按 `chrome` 分档；没有第二份 iframe 实现。
//
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 只覆盖了两面（右侧栏 / 右上浮窗）；少了 `panel` 这一档，`open` 就没法再由 mode 派生，
		...
		//   "docked"        false   true     false     并列：右侧那一列（方案 B 里接 rightbar）
		//   "floating"      false   false    true      悬浮：会话右上角浮窗（方案 B 里 DSH 提供）
		//
		...
		function nextPresentation(current, patch) {
			...
			if (trues >= 2) {
				// **旧判决的怪癖，逐字保留**：一次 patch 里给了两个 `true` 时，旧循环里
				// 第一轮会把后面那个键写成 false，而后面那一轮**只把别人写 false、不会把自己
				// 再写回 true** —— 于是净结果是三个全 false（"一次开两个"= 什么都没开）。
				// 真实调用点从不这么发（每一处都只给一个 `true`），但兼容层必须与旧行为逐字相同，
				// 否则"行为零变化"就只是"我们测到的那部分零变化"。要不要把它改成"后一个胜出"
				// 留给第二期（那是一次**有意的**行为变更，应当单独被记录与断言）。
				mode = "hidden";
		...
		//
		// 与 SURFACE_KEYS 的关系：前三个（面板 / 右侧栏 / 右上浮窗）各对应一个互斥浮层，
		// 用户切过去时由 `createUiState().set` 顺手关掉另外两个；后两个不是浮层 ——
		// 会话页签是会话主体的一部分，浏览器新页签干脆在 DSH 之外。
		var LAYOUT_PLACES = [
		 ...
		 * 切换按钮的边长与缝宽。
		 *
		 * 26 而不是工具栏那套 32：五枚连着排要挤进浮窗头部的 380px（右边还留着关闭
		 * 与名字），26×26 是"点得到"与"排得下"之间的那个数；图标 14px 与
		 ...
		 *
		 * 它是给**浮窗头部**算账用的纯函数：头部那 380px 要同时装下图标（24）、名字、
		 * 这个控件与关闭（32），名字那一格还得剩得下人话。把它抽成函数，那笔账才能在
		 * 单测里被算一遍，而不是等真机上头部挤成一条缝才发现。
		 */
		...
		function createUiState() {
			...
			var state = Object.assign({
				...
				//
				// 为什么和 mode 放在同一份模块级状态里：浮层是**从运行页的工具栏**打开的，
				// 而位置是用户在这个浮层上拖出来的，两者会交替发生；状态若活在组件内部，
				// 关掉再开就回到默认角位，用户每次都要重摆一遍。
				// 这两个键**不是"哪个面开着"**，而是浮窗的几何，所以在归一里从不参与判决
				// （混进去会让打开右侧栏顺手把用户摆好的位置清掉）。
				cornerPosition: null,
				cornerSize: null
		...
		// 四个头（全屏运行页、会话页签、右侧栏、右上浮窗）里那排按钮长得一样、干的事
		 ...
		 * 把一个小程序切到五个地方里的某一个。
		 *
		 * 三个浮层（面板 / 右侧栏 / 右上浮窗）只需要写自己那一个键：互斥是
		 ...
		 *    它们会把会话的头压得很重；
		 *  * `"none"` —— 右侧栏与右上浮窗：**不画工具栏**。它们自己有一行头
		 *    （名字 + 切换布局 + 关闭），再画一条就是两条头。
		 *
		 ...
		 * 刷新目录 → 重挂 iframe），所以：① 四个运行面（**面板 / 右侧栏 / 会话页签 / 右上浮窗**，
		 * 共用 RunnerView）行为一致；
		 * ② 发布后原地变成可运行，不需要手动刷新；③ 发布仍然是用户的**显式动作**。
		 *
		 * 刻意**不看** `chrome`：右侧栏与浮窗那两个面 `chrome === "none"`（不画工具栏），
		 * 若把按钮寄托在工具栏上，那两个面就没有出口了。
		 */
		...
		function RunnerView(props) {
			...
			// `MiniAppFloatingRunner`（column / corner 两个悬浮面）打开时调的是
			// `appCatalog.load(false)`，而 `load(false)` 的语义就是"已经加载过就不再打请求"
			// （`state.loaded === true` 直接 return）；本会话页签与全屏面板同理。
			...
			//   **只有宿主也确认"从未发布"**，才切成空态。
			//
			// 判别式放在共享的 `RunnerView` 里，所以四个运行面（同上面那四个：面板 / 右侧栏 /
			// 会话页签 / 右上浮窗）同时受益 —— 这正是
			...
			return React.createElement("div", {
				...
				toolbar ? React.createElement("div", {
					...
					React.createElement("div", {
						...
						// 「换个地方」那一排：五枚按钮（面板 / 右侧栏 / 本会话页签 / 右上浮窗 /
		...
		function MiniAppOverlay(props) {
			...
			useEffect(function () {
				 ...
				 *  * 留着它会坏两件事：之后任何一次 ui 变化（拖动浮窗写 cornerPosition、
				 *    弹一条提示、切别的面）都会重放这条命令，把用户从库页面重新拽回运行页；
				...
				// 在**这个组件还没挂载的时候**写进来的 —— 右上浮窗开着时，
				// `shell.overlay` 那个渲染口画的是浮窗（三个面互斥），全屏浮层根本
				// 没渲染。于是"从浮窗切回全屏面板"这条路上，只订阅就会把命令丢掉：
		...
		// ------------------------------------------------- 右侧栏 / 会话右上角浮窗
		//
		// 同一个组件两个 `variant`，而两者**不是同一类东西**（旧注释在这里写反了，见下）：
		//  * `column` = **右侧栏**：`position: relative` + `width/height: 100%`，填满 DSH 布局
		//    给的那一列 —— 宽高由列给，它**参与布局**，打开它会把会话挤窄；
		//  * `corner`  = **会话右上角浮窗**：`position: fixed` + 量出来的 left/top，一块约
		//    380×420 的有界窗口，浮在会话之上、**不**参与布局，所以不会让消息重排、
		//    也不会打断正在打的字（这才是"浮层"这个说法的适用面）。
		// 除了几何与锚点，两者完全一样 —— 同一行头、同一个身体（同一个 `RunnerView`）、
		// 同一套互斥语义，所以是同一个组件加一个 `variant`。
		//
		// **右侧栏这一面当前打不开，而且注释曾经与实现相反。** 旧版本这段写的是"DSH 里
		// '右侧那一列'是 `details` 座位、被 ui-conversation 的 DetailsPanel 占着，所以这里
		// **不碰** `details`，自己画一个看起来像抽屉的浮层" —— 那是上一代实现。现在代码做的
		// 恰恰是**接管 `details`**：`RIGHT_PANEL_SLOT = "details"` + `registerRightPanel()`
		// 用 `ctx.slots.inject("details", …)` 注册、并以 **priority -10** 压住 DetailsPanel
		// （`single` 座位按 priority 升序取第一个活的，工具详情那份是默认 0），
		// `openRightPanel` / `closeRightPanel` 再调 `layout.openDetails()` / `closeDetails()`。
		// 麻烦在于 0.1.5 里**这两个名字都不存在**：右列是 `rightbar` / `rightbar.session`，
		// 由 `ctx.layout.openRightbar(track, fullscreen)` 驱动，而 `details` 是幽灵名 ——
		// `ctx.slots.inject` 对不存在的名字**静默等待**，于是这一面连一次渲染机会都没有；
		// `layout.openDetails` 也被那句 `typeof … === "function"` 守卫安静跳过。
		// 两条都在 `test/seat-contract.test.mjs` 的**已知缺口账目**里钉着（那条注释原话：
		// "「切换布局 → 右侧栏」那条路从来没生效过"）。
		// **出路**：DSH 原生已经内置了同一套语义 —— `rightbar` 座位 + `ctx.sidebarRight`
		// （`isExpanded` / `toggleExpanded` / `openTab` / `float` / `dock`），push / fullscreen
		// 两种形态、折叠按钮、折叠后 header 角落的展开按钮全由它自带；接过去比继续往幽灵
		// 座位 `details` 上补更省（逐条证据见 docs/design.zh-CN.md 与 docs/research/）。
		//
		// z 轴：三个浮层都低于全屏浮层（50）—— 全屏浮层是"要看完再回来"的模态，
		// 两个贴边面（右侧栏列 / 右上角浮窗）是"边聊边用"的旁挂件；对话框（60/70）又要压在它们之上。

		/** 贴边两面的层级。低于全屏浮层的 50，高于会话内容。 */
		...
		function MiniAppFloatingRunner(props) {
			...
			/**
			 * 头部那排「切换布局」的去处：面板 / 本会话页签 / 另一个浮层 / 浏览器。
			 *
			 * 传 `props.ctx` 而不是自己取：切到本会话页签要读当前会话、并去点 DSH 画的那颗
			 * 页签，那两件事都从 ctx 进（见 `currentSessionId` / `openInSession`）。
			 *
			 * 注意这一行**同时是拖动把手**：按钮按下去必须不被 `onHeaderPointerDown` 吃掉
			 * （判定在 `isInteractiveTarget` 里），否则"点一下切过去"会变成"把窗口拖一下"。
			 */
			var onSwitchLayout = useCallback(function (place, appId) {
				switchLayout(place, appId, { t: t, ctx: props.ctx });
			}, [props.ctx, t]);
			...
			return React.createElement("div", Object.assign({
				...
				// 头部一行：名字 + 「切换布局」那一排 + 关闭。浮层的头是**它的窗框**，
				...
				//
				// 这一行里的按钮（那排切换 + 关闭）**必须点得动**：把手起手时那句
				// `preventDefault()` 会抑制 click，所以起手前先过 `isInteractiveTarget`
				// （那是一条真机 bug 的修法，回归测试钉着它）。
				React.createElement("div", {
					// 拖动这一行是把手。**四件事缺一不可**：
					//  * `onPointerDown` 起手势；
					...
					// 「切换布局」那一排。浏览器那一枚**就在这排里面**，所以原来那枚独立的
					// 「在浏览器中打开」在这里删掉了 —— 留着头部就有两个一模一样的东西。
					//
					// `layoutCurrent` 按 variant 分：浮窗是 `"corner"`，右侧栏是 `"drawer"`。
					// 两者共用这一行，所以只能这样分，不能各写一份。
					//
					// 宽度账：这一行只有 380px（`CORNER_WIDTH`），固定部分是图标 24 + 名字
					// 那一格 + 这排 5×26 + 关闭 32 + 三个缝与内边距。名字那一格是
					// `flex: "1 1 auto"` + `minWidth: 0` + 省略号，所以**被压缩的永远
					// 是名字**，这排按钮不会被挤出去（`flex: "0 0 auto"`）。
					app !== null ? React.createElement(LayoutSwitcher, {
						t: t, appId: app.miniapp_id,
						current: corner ? "corner" : "drawer",
						onSwitch: onSwitchLayout
					}) : null,
					React.createElement(ToolbarAction, {
		 ...
		 * 右侧栏里的那一格。
		 *
		 * 头（名字 + 「切换布局」 + 关闭）和浮窗共用同一段布局，身体是**同一个**
		...
		function MiniAppRightPanel(props) {
			...
			return React.createElement(MiniAppFloatingRunner, {
				...
				// ctx 要交下去：头部那排「切换布局」里"切到本会话页签"那一条靠它取会话、点页签。
				ctx: props.ctx,
