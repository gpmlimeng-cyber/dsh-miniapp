...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function switchLayout(place, appId, env) {
			...
			if (target.key === "corner") {
				...
				if (capability.column) {
					// 没有原生浮起、但有右栏通道 ⇒ 自绘浮窗外壳。写入状态即完成 ——
					// 窗口由 `shell.overlay` 座位上的 `MiniAppOverlaySeat` 按这个状态画，
					// 与全屏浮层**互斥**（`nextPresentation` 保证同一时刻只有一个面亮着）。
					ui.set({ corner: true, cornerId: appId });
					recordLayoutPlace(appId, target.key);
					return true;
				}
		...
		function MiniAppOverlaySeat(props) {
			...
			var floating = state.corner === true && state.cornerId !== null && capability.column && !capability.nativeFloat;
