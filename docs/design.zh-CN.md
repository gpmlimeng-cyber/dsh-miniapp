...
## 待排需求：入口决定默认落点（2026-09-11，用户提出）

> 同前：**需求记录**，不是实现说明。排在 t36（呈现重构第二期）之后 —— 要动同一个文件（`lib/client.js`）。

**用户原话**：「我希望弹窗的小程序默认打开显示位置在弹窗，从会话标题打开的小程序默认显示位置在会话」

### 规则（即验收）

1. **从「弹窗」打开**小程序 → 默认显示位置就在**弹窗**里；
2. **从「会话标题」打开**小程序 → 默认显示位置就在**会话**里。

### 实现含义（待确认与落地时细化）

- 打开动作需要带上**来源**（origin），由来源决定初始 `mode`（t34 已把呈现收敛为单一 `mode` + `appId`，这条正好落在它上面）；
- **不覆盖用户显式选择**：若用户手动切过形态，再次打开同一小程序时应尊重用户上次的选择，而不是无条件回到来源默认 —— **触发条件与优先级需要在实现前定清并写进验收**；
- 与「固定的小程序显示到标题栏」（上一节）是**两条独立需求**，但都落在同一个打开路径上，实现时应一起设计，避免两套"谁来定初始形态"的判决。

### 待确认（实现前必须问清）

- 「弹窗」与「会话」分别具体指哪两个呈现面？（候选：弹窗 = 浮层/浮窗；会话 = 会话内并排或会话页签）
- 用户手动切换过形态后，**来源默认**与**用户上次选择**谁优先？

### 已确认与裁定（2026-09-11）

**① 两个面已确认**（用户答复"是的"，即认可上一节的猜测）：
- **弹窗 = 全屏浮层**（`shell.overlay` 那一面 / `MiniAppOverlay`）
- **会话 = 会话内**（并列的右栏 tab，或会话页签）

**② 优先级由队长裁定：首次用来源默认，之后尊重用户上次的选择。**
- 首次（该 app **没有**用户显式切换记录）→ 用**来源决定**的默认落点（弹窗来的落弹窗、会话标题来的落会话）；
- 用户**手动切过形态**后 → **记住并按用户上次的选择**，不再每次回到来源默认。
- **理由**：前者让首次打开可预测，后者不抹掉用户的显式意图 —— 反过来（每次回到来源默认）会让用户手动切的那一下就白做了，属于"系统跟用户抢方向盘"。
- **落到实现**：需要"每个 app 的上次形态"这一份记录，建议**与「固定多个」一起设计**（两者都要动 `/prefs` 的 wire 契约与迁移），避免做两次契约变更。

---

## 收队记录：未完成事项（2026-09-11）

本轮到此收队。**主线已全部交付并提交**（`4a4ff42` 及之前 6 个提交，`pnpm test` 176/176、0 skipped）。
以下为**未完成事项**，按"能否直接开工"分类。

### A. 已识别的账（有明确做法，随时可开工）

1. **docs 口径表收口（docs-only）** —— `docs/design.zh-CN.md` 的口径表仍写着「五个地方 / 自绘浮窗」，
   而 t40 已把 647 行自绘整体退役 ⇒ **文档与实现相反**。口径同 t26/t28：只动 docs、跑 `pnpm test` 复绿即可。
2. **需求①：固定的多个小程序提升到标题栏** —— 见上文「待排需求：固定的多个小程序提升到标题栏」一节
   （四条验收 + 因"可以多个"而新增的数据模型/wire 契约/标题栏容量三件事）。
3. **需求②：入口决定默认落点** —— 见上文「待排需求：入口决定默认落点」一节
   （已确认：弹窗 = 全屏浮层、会话 = 会话内；优先级已裁定：首次用来源默认，之后尊重用户上次选择）。

### B. 要报给 DSH 上游的两条（用户侧无解，我方已绕开）

1. **技能 provider 被整条跳过** ⇒ 用户 `~/.agents/skills` 下的 **45 个技能全部取不到**。
   根因：`dsh-fs-local` 的 `probe()` 用 `stat(path,{bigint:true})` 取统计信息，而 Electron 的 asar shim
   对**归档内**路径返回的 `stats.mode` 是普通 Number ⇒ `Number(info.mode & 511n)` 抛 TypeError
   ⇒ `filesystem` 这一条来源整体被丢弃（肇事者只是 2 个内置技能）。
   **我方尝试并失败的修法**：在 profile 的 `cordis.patch.yml` 里覆盖 `skill-filesystem` 的
   `bundledSkillDir` 指向 asar 外目录 —— 配置**用应用自己的 js-yaml 校验通过**，但**重启后实测无效**
   （日志报错照旧），且肇事包在 `app.asar` 内、用户侧改不动。配置已还原为 `[]`（备份 `*.bak-162400`）。
2. **asar 打包器的 1 字节头部错误** ⇒ 危害比"manifest 读不出"更大：
   - 8/8 个 vendor `package.json` 受影响（形状 A：`size` 少 1 字节；形状 B：`offset` 早 1 字节），文件字节本身完整；
   - **第二个独立受害者（t33 实测）**：按头部偏移抽出的 `dsh` CLI **无法启动** ⇒ "按归档头部抽出来的可执行文件是坏的"。

### C. 已知的可选残留

- t40 附带删除了 `isInteractiveTarget`（不在当时验收清单内）：它唯一的调用方是已删除的三个手势，
  删后无调用点、无导出、无测试。如需保守可回滚，但它不进任何断言。
- `/tmp` 下的若干快照与探针（`t39_prefix_*.bak`、`t40_prefix/`、`t22prep/`、`t33/`）随会话结束失效；
  **凡进入结论的证明都已落进本文件或提交历史**（口径第 10 条）。

### D. 本轮沉淀的口径（14 条，均来自具体事故）

```
 1 结论要带来源、口径、读取时刻；状态陈述带 updatedAt
 2 证据不得跨 revision 复用
 3 判红也要带证据（哪条判据、哪次变异、命令与输出）
 4 变异前存快照；禁用 git checkout --；快照必须带版本标签
 5 评审者的变异属临时操作：备份 + 逐字节还原 + 显式声明
 6 还原后必须用哈希对冻结指纹校验（不得假定 cp 回去就对）
 7 先怀疑测量工具，再怀疑被测代码
 8 锚定分两个量：行为锚（文件哈希）+ 身份锚（骨架哈希）
 9 冻结声明 = 文件清单 + 每文件哈希 + "清单内文件一变，结论即失效"
10 一次性证明永久留证（含归一化规则）；快照删除只能在证明落记录之后
11 扩大交付面必须建 inScope 正确的新任务（消息授权在工具层不生效）
12 收到派单先看状态；终态不重做；重派后必须重新 claim（**双向**：派单侧发单前也先看状态）
13 有成员在飞时不得抢提交工作区（禁用 git add -A）
14 inScope 必须包含"会被这次改动反向约束的账目文件"（锚文件 + 席位账目文件）
```
